...
export function SpatialStudio({
  ...
  return (
    <div className={`w-full h-screen bg-cosmic-void ${className}`}>
      {/* Control Panel */}
      <div className="absolute top-4 left-4 z-10 glass-strong rounded-xl p-4 border border-white/10">
        <div className="flex items-center gap-2 mb-3">
          ...
          {isXRActive && (
            <Badge variant="crystal" className="text-xs">VR/AR MODE</Badge>
          )}
        </div>
        ...
        {enablePerformanceMonitoring && (
          <div className="mb-3 p-2 bg-white/[0.03] rounded-lg">
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div>
                <span className="text-text-muted">FPS:</span>
                <span className={`ml-1 font-mono ${
                  performanceStats.fps >= 50 ? 'text-arcane-earth' : 'text-arcane-fire'
                }`}>
        ...
        <div className="space-y-1">
          <div className="text-text-secondary text-xs">Controls:</div>
          <div className="text-text-disabled text-xs">Click to interact</div>
          <div className="text-text-disabled text-xs">Drag to rotate</div>
          <div className="text-text-disabled text-xs">Scroll to zoom</div>
          {isXRActive && (
            <>
              <div className="text-text-disabled text-xs">Use controllers</div>
              <div className="text-text-disabled text-xs">Hand tracking</div>
            </>
          )}
        </div>
      </div>
...
function GuardianStatusPanel({ performanceStats }: GuardianStatusPanelProps) {
  ...
  return (
    <div className="absolute bottom-4 right-4 z-10 glass-strong rounded-xl p-4 border border-white/10 min-w-80">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-arcane-crystal font-display text-sm">Guardian Council</h3>
        {performanceStats && (
          <Badge variant={performanceStats.fps >= 50 ? 'gold' : 'crystal'} className="text-xs">
        ...
        {activeGuardians.map((guardian) => (
          <div key={guardian.name} className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <span className="text-text-secondary text-sm font-medium">{guardian.name}</span>
              <span className="text-text-disabled text-xs ml-auto">{guardian.frequency} Hz</span>
            </div>

            <div className="flex items-center gap-2">
              <div className="flex-1 bg-white/[0.06] rounded-full overflow-hidden h-2">
                <div
      ...
      <div className="mt-4 pt-3 border-t border-white/10">
        <div className="flex items-center justify-between text-xs">
          <span className="text-text-muted">Harmony Level</span>
          <div className="flex items-center gap-2">
            <div className="flex gap-1">
              {['fire', 'water', 'earth', 'void'].map((element, i) => (
                <div
                  key={element}
                  className={`w-2 h-2 rounded-full ${getElementColor(element)} opacity-80`}
                  style={{
