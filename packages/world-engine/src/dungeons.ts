...
const DUNGEON_RIGHTS_NOTES: Record<DungeonRightsTier, string> = {
  ...
  factual_reference_only:
    "Protected source reference. Store factual metadata and generate only the original Arcanea variant.",
  blocked: "Blocked for generation or publication until reviewed.",
};

const DUNGEON_POLICY_NOTES: Record<DungeonGenerationPolicy, string> = {
...
export function canGenerateDungeonArt(entry: DungeonAtlasEntry): boolean {
  return (
...
export function getDungeonAtlasSafetyNotes(entry: DungeonAtlasEntry): string[] {
  const notes = [
  ...
  if (entry.source.referenceMode === "factual_reference") {
    notes.push(
      `Reference source is ${entry.source.sourceWork}; keep source names and citations in metadata, not in generated artwork prompts.`,
    );
  }
...
export function dungeonAtlasEntryToPromptPack(entry: DungeonAtlasEntry): DungeonPromptPack {
...
export function getDungeonMinutesRemaining(entry: DungeonAtlasEntry, elapsedMinutes: number): number {
  return Math.max(0, entry.arcaneaVariant.timer.resonanceWindowMinutes - elapsedMinutes);
}

export function getDungeonCollapseStage(
  entry: DungeonAtlasEntry,
  elapsedMinutes: number,
): DungeonCollapseStage | undefined {
...
export function canChallengeDungeonBoss(
  entry: DungeonAtlasEntry,
...
export function evaluateDungeonRun(
  entry: DungeonAtlasEntry,
  ...
  let verdict = "in_progress" as DungeonRunResult["verdict"];
