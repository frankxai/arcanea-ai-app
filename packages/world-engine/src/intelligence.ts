...
export function analyzeElementalBalance(nodes: CreationNode[]): {
  ...
  for (const node of nodes) {
    const elem =
      (node.element as string | undefined) ??
      (node.metadata?.primaryElement as string | undefined) ??
      (node.metadata?.dominantElement as string | undefined);
...
export function detectRoles(nodes: CreationNode[]): {
  filled: string[];
  missing: string[];
  duplicates: string[];
} {
  ...
  for (const char of characters) {
    const text =
      JSON.stringify(char.metadata).toLowerCase() + " " +
      char.name.toLowerCase() + " " +
      ((char.metadata?.rank as string | undefined) ?? "").toLowerCase();
...
export function detectGaps(nodes: CreationNode[], edges: CreationEdge[]): WorldGap[] {
  const gaps: WorldGap[] = [];
  const characters = nodes.filter(n => n.type === "character");
  ...
  const gateLevels = characters.map(c =>
    (c.gate as number | undefined) ??
    (c.metadata?.gatesOpen as number | undefined) ??
    1
  );
...
export function weaveNarrative(
  ...
  const protGate    = (protagonist.gate as number | undefined) ?? (protagonist.metadata?.gatesOpen as number | undefined) ?? 1;
  ...
  return {
    title: `The ${(protagonist.metadata?.rank as string | undefined) ?? "Master"}'s Dilemma`,
    type: "transformation",
    ...
    acts: [
      {
        act: 1,
        title: "The Weight",
        summary: `${protagonist.name}, a respected ${(protagonist.metadata?.rank as string | undefined) ?? "Master"}, discovers a crack in the Gate system — a vulnerability that ${antagonist.name} is already exploiting.`,
        involvedCreations: [protagonist.name, antagonist.name],
...
export function generateWorldReport(
  nodes: CreationNode[],
  edges: CreationEdge[],
): WorldReport {
  ...
  const nextActions = gaps
    .sort((a, b) => {
      const sev = { critical: 0, important: 1, nice_to_have: 2 } as const;
