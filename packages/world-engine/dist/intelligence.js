...
// ── Quest Generator ──────────────────────────────────────────────────────────
export function generateQuest(nodes, _edges) {
    const characters = nodes.filter(n => n.type === "character");
    const locations = nodes.filter(n => n.type === "location");
    if (nodes.length < 2)
        return null;
    ...
    return {
        ...
        estimatedDifficulty: avgGate <= 2 ? "apprentice" :
            avgGate <= 4 ? "mage" :
                avgGate <= 6 ? "master" :
...
// ── Faction Dynamics ─────────────────────────────────────────────────────────
export function analyzeFactions(nodes, _edges) {
    const characters = nodes.filter(n => n.type === "character");
    const elementGroups = new Map();
    characters.forEach(c => {
        const elem = c.element ??
    ...
    const factions = [...elementGroups.entries()].map(([element, members]) => {
        ...
        const hasVillain = members.some(m => JSON.stringify(m.metadata).toLowerCase().includes("dark") ||
    ...
    for (let i = 0; i < factions.length; i++) {
        for (let j = i + 1; j < factions.length; j++) {
            ...
            else if (f1.alignment !== f2.alignment &&
