...
const RIGHTS_NOTES = {
    ...
    factual_reference_only: "Protected source reference. Use factual metadata only and generate an original Arcanea variant.",
...
export function canGenerateCreatureImage(entry) {
...
export function getCreatureAtlasSafetyNotes(entry) {
    const notes = [
    ...
    if (entry.source.referenceMode === "factual_reference") {
        notes.push(`Reference source is ${entry.source.sourceWork}; keep names and citations in metadata, not in the generated image prompt.`);
    }
    return notes;
}
export function creatureAtlasEntryToPromptPack(entry) {
    const variant = entry.arcaneaVariant;
    const factualSource = entry.source.referenceMode === "factual_reference"
