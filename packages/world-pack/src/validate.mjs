...
export function validatePack(pack) {
  ...
  req(
    SEMVER_RE.test(pack.packVersion || ""),
    "packVersion must be semver x.y.z",
  );
  req(
    pack.world && WORLD_ID_RE.test(pack.world.id || ""),
    "world.id must match wld_<26 Crockford base32>",
  );
  req(
    typeof pack.world?.name === "string" && pack.world.name.length >= 2,
    "world.name must be at least 2 characters",
  );
  req(
    typeof pack.world?.creatorRef === "string",
    "world.creatorRef is required",
  );
  req(
    pack.canon && typeof pack.canon.document === "string",
    "canon.document is required",
  );
  if (pack.canon?.sourceHash != null)
    req(
      HASH_RE.test(pack.canon.sourceHash),
      "canon.sourceHash must be sha256:<hex>",
    );
  if (pack.digest != null)
  ...
  for (const node of pack.nodes || []) {
    ...
    ids.add(node?.id);
    req(
      ENTITY_TYPES.includes(node?.type),
      `${at}: type must be one of ${ENTITY_TYPES.join(", ")}`,
    );
    req(
    ...
    req(
      LAYERS.includes(node?.layer),
      `${at}: layer must be one of ${LAYERS.join(", ")}`,
    );
    const g = node?.governance;
    req(g && typeof g === "object", `${at}: governance envelope is required`);
    if (!g) continue;
    req(
      typeof g.owner === "string" && g.owner.length > 0,
      `${at}: governance.owner is required`,
    );
    req(
      VISIBILITIES.includes(g.visibility),
      `${at}: governance.visibility invalid`,
    );
    req(
      CANON_STATUSES.includes(g.canonStatus),
      `${at}: governance.canonStatus invalid`,
    );
    req(
      RIGHTS_STATES.includes(g.rights?.state),
      `${at}: governance.rights.state invalid`,
    );
    req(
      typeof g.evalRule === "string" && g.evalRule.length > 0,
      `${at}: governance.evalRule is required`,
    );
    if (node.provenance?.promptHash != null)
      req(
        HASH_RE.test(node.provenance.promptHash),
        `${at}: provenance.promptHash must be sha256:<hex>`,
      );
  }
  ...
  for (const rel of pack.relationships || []) {
    ...
    req(
      RELATIONSHIP_KINDS.includes(rel?.kind),
      `${at}: kind must be one of ${RELATIONSHIP_KINDS.join(", ")}`,
    );
    req(
      typeof rel?.from === "string" && typeof rel?.to === "string",
      `${at}: from and to are required`,
    );
    if (rel?.strength != null)
      req(
        rel.strength >= 0 && rel.strength <= 1,
        `${at}: strength must be 0..1`,
      );
    req(
      rel?.governance && typeof rel.governance === "object",
      `${at}: governance envelope is required`,
    );
  }
  ...
  for (const v of versions) {
    ...
    req(
      ...
      `${at}: createdAt is required`,
    );
    if (v?.digest != null)
      req(HASH_RE.test(v.digest), `${at}: digest must be sha256:<hex>`);
  }

  for (const s of sources)
    req(
      typeof s?.id === "string" && typeof s?.kind === "string",
      `source ${s?.id ?? "<no id>"}: id and kind are required`,
    );
