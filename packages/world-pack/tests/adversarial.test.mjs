...
test("a user node named Lyria that declares layer:'canon' is blocked, not blessed", async () => {
  ...
  assert.ok(
    rules.has("canon.layer-claim"),
    `expected canon.layer-claim, got ${[...rules].join(", ")}`,
  );
  assert.ok(
    rules.has("canon.locked-name-taken"),
    "the locked-name rule applies whatever layer the node declares",
  );
...
test("the real canon-layer Universe node is attested and stays clean", async () => {
  ...
  const stolen = {
    ...
    nodes: pack.nodes.map((n) =>
      n.type === "Universe"
        ? { ...n, governance: { ...n.governance, owner: "adv" } }
        : n,
    ),
  };
...
test("an incoming branch node that keeps layer:'canon' still cannot rewrite the universe", async () => {
  ...
  const hostile = {
    ...
    nodes: forked.nodes.map((n) =>
      n.id === universeId
        ? { ...n, name: "Arcanea (rewritten by attacker)" }
        : n,
    ),
  };
  ...
  for (const opts of [
    ...
    assert.equal(
      blocked.pack.nodes.find((n) => n.id === universeId).name,
      "Arcanea",
    );
  }
});
...
test("a pack that records an all-zero canon hash does not clear canon", async () => {
  ...
  const hit = report.findings.find(
    (f) => f.ruleId === "canon.binding-mismatch",
  );

  assert.ok(
    hit,
    "a pack cleared against a hash it wrote itself is cleared against nothing",
  );
  ...
  assert.ok(
    rulesOf(detectConflicts(unbound, canon)).has("canon.binding-mismatch"),
  );
});
...
async function exported() {
  const { canon, pack } = await seed();
  const built = commit(withGuardianRoles(pack), {
    message: "seed",
    by: pack.world.creatorRef,
    at: AT,
  });
...
test("verifyExport catches an edited createdBy, citation, agentRole or count", async () => {
  ...
  const mutations = {
    createdBy: {
      ...
      provenance: {
        ...e.provenance,
        versions: e.provenance.versions.map((v) => ({
          ...v,
          createdBy: "attacker",
        })),
      },
    ...
    citation: {
      ...
      provenance: {
        ...e.provenance,
        sources: e.provenance.sources.map((s) => ({
          ...s,
          citation: "Authored by @attacker",
        })),
      },
    },
    agentRoles: {
      ...
      agentRoles: [
        {
          ...GUARDIAN_ROLES[5],
          id: "guardian.attacker",
          authority: {
            scopes: ["canon:approve-locked"],
            owns: [],
            maxWaivableSeverity: "blocker",
          },
        },
      ],
    },
  ...
  for (const [what, mutated] of Object.entries(mutations)) {
    assert.equal(
      verifyExport(mutated).valid,
      false,
      `${what} was edited after export and verifyExport still said valid`,
    );
  }
});
...
test("re-signing a forged authority model does not make it valid", async () => {
  ...
  assert.equal(
    result.agentRolesOk,
    false,
    "authority is defined in code, not asserted by a file",
  );
  assert.equal(result.valid, false);
  assert.ok(
    rulesOf(detectConflicts(forged, canon)).has(
...
test("a locked truth broken in attributes.backstory is caught like one broken in description", async () => {
  ...
  assert.ok(
    rules.has("canon.nero-miscast"),
    "choosing a different key is not a way past the rule",
  );
...
test("unknown enum values are findings, not silence", async () => {
  ...
  for (const expected of [
    ...
    assert.ok(
      rules.has(expected),
      `expected ${expected}, got ${[...rules].join(", ")}`,
    );
  }
});
...
test("a locked name under a hat, or in an alias, is still that name", async () => {
  ...
  const hit = report.findings.find(
    (f) => f.ruleId === "canon.alias-of-locked-name",
  );
  assert.ok(
    hit,
    `expected canon.alias-of-locked-name, got ${[...rulesOf(report)].join(", ")}`,
  );
  ...
  assert.ok(
    rulesOf(detectConflicts(addNode(pack, accented), canon)).has(
...
test("checkAgainst runs the same rules on a creator's own canon document", async () => {
  ...
  const contradicts = {
    ...
    layer: "user",
    attributes: {
      backstory: "Maren is a goddess who wants the ships that cross her.",
    },
    governance: { ...govOf(pack), canonStatus: "draft" },
  };
  ...
  assert.ok(
    rules.has("canon.locked-name-taken"),
    `'Maren' is locked in the creator's own canon; got ${[...rules].join(", ")}`,
  );
  assert.ok(rules.has("canon.locked-truth-contradiction"));
  assert.ok(
    rules.has("canon.binding-foreign"),
    "checking against a foreign canon is reported, not enforced",
  );
  ...
  assert.equal(
    report.findings.filter(
      (f) => f.nodeName === "Arcanea" && f.severity !== "info",
    ).length,
    0,
  );
