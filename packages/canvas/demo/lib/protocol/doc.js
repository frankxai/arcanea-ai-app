import { applyOp, cloneState, emptyState, fromSnapshot } from "./reducer.js";
const FRAME_MS = 16;
/**
...
export class CanvasDoc {
    ...
    transport;
    events;
    constructor(clientId, transport, events = {}) {
    ...
    receive(message) {
        switch (message.kind) {
            ...
            case 'op_status': {
                ...
                if (message.seq > this.confirmed.seq)
                    this.confirmed.seq = message.seq;
                break;
            }
            case 'reject': {
                this.drop(message.opId);
