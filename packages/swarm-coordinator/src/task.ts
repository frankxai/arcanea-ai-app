 ...
 * Task Domain Entity
 *
 * Represents a task to be executed by agents. Supports priority-based sorting,
 * topological dependency resolution, lifecycle management, and nested workflows.
 *
 * Absorbed from arcanea-flow V3 task-execution/domain/Task.ts
 */
...
export class Task implements TaskConfig {
  ...
  getPriorityValue(): number {
    const values: Record<TaskPriority, number> = { high: 3, medium: 2, low: 1 };
  ...
  /**
   * Topological sort — resolve execution order respecting dependencies.
