...
export const LUMINOR_HINTS: Record<string, LuminorSpec> = {
...
export const GUARDIANS: GuardianInfo[] = [
 ...
 */
export const AGENT_GUARDIAN_MAP: Record<string, { primaryGuardian: string; secondaryGuardians: string[] }> = {
 ...
 */
export function classifyCoordinationMode(weights: Record<string, number>): CoordinationMode {
 ...
 */
export function resolveSwarm(weights: Record<string, number>): SwarmResult {
...
export function getGuardianBySlug(slug: string): GuardianInfo | undefined {
  return GUARDIANS.find(g => g.slug === slug);
}

export function getGuardianByGate(gate: number): GuardianInfo | undefined {
 ...
 */
export function getAgentSwarmInfo(agentId: string): {
  primaryGuardian: GuardianInfo | undefined;
  secondaryGuardians: GuardianInfo[];
  luminorTeam: Array<{ id: string; hint: string; team: string }>;
  ...
  const secondaries = mapping.secondaryGuardians
    ...
    .filter(Boolean) as GuardianInfo[];
  ...
  const luminorTeam = luminorIds
    ...
    .filter(Boolean) as Array<{ id: string; hint: string; team: string }>;
...
export function resolveForMode(
  ...
  if (mode === 'council' && primaryGuardianSlug) {
    ...
    if (!guardian) return resolveSwarm({});
    // Build weights: primary at 0.7, neighbors at 0.5
    const gateIndex = guardian.gate - 1;
    const weights: Record<string, number> = { [primaryGuardianSlug]: 0.7 };
    if (gateIndex > 0) weights[GUARDIANS[gateIndex - 1].slug] = 0.5;
    if (gateIndex < GUARDIANS.length - 1) weights[GUARDIANS[gateIndex + 1].slug] = 0.5;
