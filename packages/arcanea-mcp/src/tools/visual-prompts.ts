...
// Element → visual language mapping
const ELEMENT_AESTHETICS: Record<string, { colors: string; atmosphere: string; materials: string; lighting: string }> = {
  Fire: {
...
// Rank → visual power level
const RANK_VISUAL: Record<string, { complexity: string; aura: string; attire: string }> = {
  Apprentice: {
...
interface CharacterBlueprint {
  ...
  rank: string;
  gatesOpen: number;
  patronGuardian?: { name: string; domain: string };
  godbeast?: { name: string; form: string; bond?: string };
 ...
 */
export function characterToImagePrompt(char: CharacterBlueprint, style?: string): string {
  ...
  const stylePrefix = style || "digital painting, concept art, fantasy character portrait";
  ...
  // Negative prompt
  const negative = "text, watermark, blurry, low quality, anime unless specified, modern clothing, guns, technology";
 ...
 */
export function locationToImagePrompt(loc: LocationBlueprint, style?: string): string {
  const elem = ELEMENT_AESTHETICS[loc.dominantElement] || ELEMENT_AESTHETICS.Earth;
  const stylePrefix = style || "digital painting, fantasy landscape, concept art, wide shot";

  const alignmentMood = loc.alignment === "light"
    ? "hopeful, sacred, welcoming, golden light"
    : loc.alignment === "dark"
    ? "ominous, ancient, foreboding, deep shadows"
  ...
  return JSON.stringify({
    prompt: parts.join(", "),
    negativePrompt: "text, watermark, blurry, modern buildings, cars, technology",
    suggestedModels: ["SDXL", "Flux", "Midjourney v6"],
 ...
 */
export function creatureToImagePrompt(creature: CreatureBlueprint, style?: string): string {
  const elem = ELEMENT_AESTHETICS[creature.element] || ELEMENT_AESTHETICS.Fire;
  const stylePrefix = style || "digital painting, creature design, fantasy illustration, concept art";
  ...
  return JSON.stringify({
    prompt: parts.join(", "),
    negativePrompt: "text, watermark, blurry, cute cartoon style, chibi, human",
    suggestedModels: ["SDXL", "Flux", "Midjourney v6"],
