...
export function generateQuest(sessionId: string): Quest | null {
  const nodes = getGraphNodes(sessionId);
  const characters = nodes.filter(n => n.type === "character");
  const locations = nodes.filter(n => n.type === "location");
  const artifacts = nodes.filter(n => n.type === "artifact");
  ...
  const questTypes: Quest["type"][] = ["fetch", "escort", "investigate", "defend", "discover", "negotiate", "survive"];
...
interface FactionReport {
  ...
  tensions: Array<{ between: [string, string]; reason: string; severity: "low" | "medium" | "high" | "war" }>;
  powerBalance: string;
  prediction: string;
}

export function analyzeFactions(sessionId: string): FactionReport {
  const nodes = getGraphNodes(sessionId);
  const edges = getGraphEdges(sessionId);
  const characters = nodes.filter(n => n.type === "character");
  ...
  const oppositions: Record<string, string> = { Fire: "Water", Water: "Fire", Earth: "Wind", Wind: "Earth", Void: "Spirit", Spirit: "Void" };
  const tensions: FactionReport["tensions"] = [];

  for (let i = 0; i < factions.length; i++) {
    for (let j = i + 1; j < factions.length; j++) {
      const f1 = factions[i]; const f2 = factions[j];
