...
import { WorldMaturity, matchCreativeSkill, } from "./types.js";
 ...
 */
export function assessWorldState(sessionId) {
 ...
 */
export function intentGate(request) {
 ...
 */
export function assessRequest(request, worldState) {
 ...
 */
export function createAgentTasks(decision, request, sessionId) {
    const tasks = [];
 ...
 */
export async function executeTasks(tasks, parallel) {
    if (parallel) {
 ...
 */
export function synthesizeResults(tasks, worldState) {
 ...
 */
export async function orchestrateCreativeSession(request, sessionId = "default") {
    // Create session
    const session = {
        id: `session-${Date.now()}`,
        goal: request,
...
export function getActiveSessions() {
    return Array.from(activeSessions.values()).filter(s => s.state !== "complete" && s.state !== "failed");
}
/**
 * Get task by ID
 */
