...
export class SkillRulesEngine {
    ...
    activate(input, context) {
        ...
        if (this.config.cascadeEnabled) {
            for (const cascade of this.cascadeRules) {
                if (this.shouldCascade(cascade, activatedRules)) {
                    const cascadeRule = this.rules.find((r) => r.skill === cascade.also);
                    if (cascadeRule &&
        ...
        const reasoning = activated
            .map((m) => `${m.rule.skill} [${m.matchType}: "${m.matchedOn}"] → ${m.rule.guardian}`)
            .join("; ");
        return {
            activated: activatedRules,
            guardian,
            gate: guardianInfo?.gate || primary?.gate || this.config.defaultGate,
            frequency: guardianInfo?.frequency || primary?.frequency || "1111 Hz",
            cascaded,
     ...
     */
    shouldCascade(cascade, activated) {
        const when = cascade.when.toLowerCase();
        // "any rule with gate X activates"
        const gateMatch = when.match(/gate\s+([\w,\s]+(?:or\s+\w+)?)\s+activates/);
        if (gateMatch) {
        ...
        const alongsideMatch = when.match(/gate\s+(\w+)\s+activates\s+alongside\s+gate\s+(\w+)/);
        if (alongsideMatch) {
            const gateA = alongsideMatch[1].toLowerCase();
            const gateB = alongsideMatch[2].toLowerCase();
            return (activated.some((r) => r.gate.toLowerCase() === gateA) &&
     ...
     */
    recordFeedback(executionId, feedback) {
     ...
     */
    getGuardianHistory(guardian) {
        return this.executionLog.filter((e) => e.activation.guardian === guardian);
    }
    /**
...
// --- Singleton ---
let _instance = null;
export function getSkillRulesEngine() {
    if (!_instance) {
        _instance = new SkillRulesEngine();
    }
    return _instance;
}
//# sourceMappingURL=skill-rules-engine.js.map