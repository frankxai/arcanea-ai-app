...
function identifyPatterns(symptoms, context) {
...
function generateThinkingProcess(symptoms, context, depth) {
...
function getLuminorForPattern(patterns) {
...
export async function deepDiagnosis(symptoms, context, history, depth = "standard") {
    const patterns = identifyPatterns(symptoms, context || "");
    const thinkingProcess = generateThinkingProcess(symptoms, context || "", depth);
    // Find primary creature
    let primaryCreature = null;
    ...
    if (!primaryCreature) {
        ...
        for (const [slug, creature] of Object.entries(bestiary)) {
            const matchCount = creature.symptoms.filter(s => lowerSymptoms.includes(s.toLowerCase().split(" ")[0])).length;
            if (matchCount > 0) {
...
//# sourceMappingURL=deep-diagnosis.js.map