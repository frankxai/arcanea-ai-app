...
export async function validateCanon(content, contentType) {
    ...
    for (const [guardian, gate] of Object.entries(guardianGateMap)) {
        if (lowerContent.includes(guardian)) {
            ...
            const nearbyText = lowerContent.substring(Math.max(0, guardianIndex - 50), Math.min(lowerContent.length, guardianIndex + 50));
            for (let i = 1; i <= 10; i++) {
    ...
    if (contentType === "story") {
        suggestions.push("Use 'the Song' or 'the Arcane' for formal magic references", "Show magic through effect, not technical description", "Reference the Arc for cycles of creation and destruction");
    }
    else if (contentType === "character") {
        suggestions.push("Check that any Guardian/Godbeast names are spelled correctly", "Ensure elemental affiliations are from the Five Elements", "Verify magic rank matches Gates opened (0-2: Apprentice, 3-4: Mage, etc.)");
    }
