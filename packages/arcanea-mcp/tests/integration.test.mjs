...
// A declared test corpus keeps library checks independent of checkout layout.
const libraryFixture = mkdtempSync(
  join(tmpdir(), "arcanea-integration-library-"),
);
const previousLibraryDirectory = process.env.ARCANEA_LIBRARY_DIR;
mkdirSync(join(libraryFixture, "fixture-collection"));
writeFileSync(
  join(libraryFixture, "fixture-collection", "sample.md"),
  "# Test reference\n\nGuardian gate creation fire water Lumina Nero Malachar. This is synthetic test input.\n",
);
process.env.ARCANEA_LIBRARY_DIR = libraryFixture;
after(() => {
  if (previousLibraryDirectory === undefined)
    delete process.env.ARCANEA_LIBRARY_DIR;
  else process.env.ARCANEA_LIBRARY_DIR = previousLibraryDirectory;
  assert.ok(resolve(libraryFixture).startsWith(resolve(tmpdir()) + sep));
  rmSync(libraryFixture, { recursive: true, force: true });
});
...
function parseContent(result) {
  ...
  assert.ok(
    ...
    "result.content must have at least one item",
  );
  assert.equal(
...
describe("Memory helpers used by diagnostics", () => {
  // These assertions exercise the memory helpers, not a registered health tool.
  ...
  it("memory file follows the configured data directory or OS home default", () => {
    const path = memIndex.getMemoryFilePath();
    assert.ok(
      typeof path === "string",
      "getMemoryFilePath should return a string",
    );
    const dataRoot =
      process.env.ARCANEA_DATA_DIR || join(homedir(), ".arcanea");
    assert.equal(path, join(dataRoot, "memories.json"));
  });
...
describe("Academy Progress — get_academy_progress canonical ranks", () => {
  ...
  for (const { gatesOpen, expected } of rankSystem) {
    it(`${gatesOpen} gates open → rank ${expected}`, () => {
      ...
      const matched = thresholds.find(
        (t) => gatesOpen >= t.min && gatesOpen <= t.max,
      );
      assert.ok(matched, `No rank matched for gatesOpen=${gatesOpen}`);
      assert.equal(
        matched.rank,
        expected,
        `gatesOpen=${gatesOpen} should be ${expected}`,
      );
    });
  }
  ...
  it("rank thresholds cover all 0–10 gate values with no gaps", () => {
    ...
    for (let gates = 0; gates <= 10; gates++) {
      const matched = thresholds.filter(
        (t) => gates >= t.min && gates <= t.max,
      );
      assert.equal(
        matched.length,
        1,
        `Gates=${gates} should match exactly 1 threshold, got ${matched.length}`,
      );
    }
  });
  ...
  it("rank thresholds have no overlaps", () => {
    ...
    for (let i = 0; i < thresholds.length - 1; i++) {
      assert.ok(
        thresholds[i].max < thresholds[i + 1].min,
        `Overlap between rank ${thresholds[i].rank} and ${thresholds[i + 1].rank}`,
      );
    }
  });
  ...
  it("Luminor is the terminal rank (no next rank)", () => {
    ...
    assert.equal(
      luminorIndex,
      thresholds.length - 1,
      "Luminor should be the last rank",
    );
  });
  ...
  it("academy progress integrates with memory session", async () => {
    const { getOrCreateSession, recordGateExplored } =
    ...
    const rank = thresholds.find(
      (t) =>
        session.gatesExplored.length >= t.min &&
        session.gatesExplored.length <= t.max,
    );
  ...
  it("unexplored gate suggestions returns at most 3 entries", () => {
    ...
    const unexplored = allGates
      .filter((g) => !explored.has(g.gate))
      .slice(0, 3);
    assert.ok(
      unexplored.length <= 3,
      "Should return at most 3 suggested next gates",
    );
...
describe("Library Search — searchLibrary function", () => {
  ...
  it("relevanceScore is a positive number", async () => {
    ...
    for (const r of data.results) {
      assert.ok(
        ...
        "relevanceScore should be a number",
      );
  ...
  it("limit parameter caps results", async () => {
    ...
    assert.ok(
      ...
      `Expected <= 3 results, got ${data.results.length}`,
    );
  });
  ...
  it("limit defaults to 5", async () => {
    ...
    assert.ok(
      ...
      `Default limit should be 5, got ${data.results.length}`,
    );
  });
  ...
  it("limit of 20 is the maximum", async () => {
    const data = parseContent(
      await searchLibrary("guardian creation lumina nero", 20),
    );
  ...
  it("empty query returns error object", async () => {
    ...
    assert.ok(
      "error" in data || data.results.length === 0,
      "Empty/whitespace query should return error or no results",
    );
  });
  ...
  it("single-character query returns error (too short for indexing)", async () => {
    ...
    assert.ok(
      "error" in data,
      "Single-character query should return an error about minimum keyword length",
    );
  });
  ...
  it("file paths use forward slash separators", async () => {
    ...
    for (const r of data.results) {
      assert.ok(
        !r.file.includes("\\"),
        `File path should use forward slashes: ${r.file}`,
      );
    }
  });
  ...
  it("excerpt length does not exceed 350 characters (including ellipsis)", async () => {
    ...
    for (const r of data.results) {
      assert.ok(
        r.excerpt.length <= 350,
        `Excerpt too long: ${r.excerpt.length} chars`,
      );
    }
  });
  ...
  it("totalMatches >= returned count", async () => {
    ...
    assert.ok(
      data.totalMatches >= data.returned,
      "totalMatches should be >= returned",
    );
  ...
  it("multi-keyword search finds more matches than single keyword", async () => {
    ...
    const multi = parseContent(
      await searchLibrary("guardian gate creation lumina"),
    );
  ...
  it("no-results case returns informative message", async () => {
    ...
    if (data.results.length === 0) {
      assert.ok(
        data.message.includes("No matching") ||
          data.message.includes("Found 0"),
...
The MCP Server itself makes no outbound network requests.`;
    ...
    assert.ok(
      lower.includes("does not collect") || lower.includes("not collect"),
      "Policy must state data is not collected",
    );
    // Must mention local storage
    assert.ok(
      lower.includes("localstorage") || lower.includes("local storage"),
      "Policy must mention local storage",
    );
    // Must mention no network requests
    assert.ok(
      lower.includes("no outbound") || lower.includes("makes no"),
      "Policy must state no outbound network requests",
    );
  });
  ...
  it("privacy policy mentions Chrome extension", () => {
    const text =
      "Arcanea Chrome Extension does NOT collect personal information.";
  ...
  it("privacy policy mentions local file path", () => {
    const text =
      "Stores session data ONLY in a local file (~/.arcanea/memories.json).";
...
describe("Error Handling — generators with boundary inputs", () => {
  ...
  it("generateCharacter with gatesOpen=1 produces Apprentice rank", async () => {
    const data = JSON.parse(
      (await generateCharacter({ gatesOpen: 1 })).content[0].text,
    );
  ...
  it("generateCharacter with gatesOpen=10 produces Luminor rank", async () => {
    const data = JSON.parse(
      (await generateCharacter({ gatesOpen: 9 })).content[0].text,
    );
  ...
  it("generateCharacter returns all 6 valid elements without throwing", async () => {
    for (const element of ["Fire", "Water", "Earth", "Wind", "Void"]) {
      const data = JSON.parse(
        (await generateCharacter({ primaryElement: element })).content[0].text,
      );
  ...
  it("generateCharacter with all 7 houses produces valid output", async () => {
    ...
    for (const house of houses) {
      const data = JSON.parse(
        (await generateCharacter({ house })).content[0].text,
      );
  ...
  it("generateMagicAbility at gate 1 has no anima cost", async () => {
    const data = JSON.parse(
      (await generateMagicAbility({ element: "Fire", gateLevel: 1 })).content[0]
        .text,
    );
    assert.equal(
      data.cost.anima,
      undefined,
      "Low-gate ability should have no anima cost",
    );
  });

  it("generateMagicAbility at gate 10 has anima cost", async () => {
    const data = JSON.parse(
      (await generateMagicAbility({ element: "Spirit", gateLevel: 10 }))
        .content[0].text,
    );
    assert.ok(
      data.cost.anima !== undefined,
      "High-gate ability should have anima cost",
    );
  });
  ...
  it("generateName with count=20 returns exactly 20 names", async () => {
    const data = JSON.parse(
      (await generateName({ count: 20 })).content[0].text,
    );
  ...
  it("generateCreature with all valid sizes returns correct size", async () => {
    for (const size of ["tiny", "small", "medium", "large", "massive"]) {
      const data = JSON.parse(
        (await generateCreature({ size })).content[0].text,
      );
  ...
  it("generateCreature with all temperaments returns correct temperament", async () => {
    for (const temperament of ["hostile", "neutral", "friendly", "sacred"]) {
      const data = JSON.parse(
        (await generateCreature({ temperament })).content[0].text,
      );
  ...
  it("generateArtifact with all power levels returns valid output", async () => {
    for (const power of ["minor", "moderate", "major", "legendary"]) {
      const data = JSON.parse(
        (await generateArtifact({ power })).content[0].text,
      );
  ...
  it("generateLocation with all alignments returns correct alignment", async () => {
    for (const alignment of ["light", "dark", "balanced"]) {
      const data = JSON.parse(
        (await generateLocation({ alignment })).content[0].text,
      );
  ...
  it("generateStoryPrompt with all 10 gate numbers returns correct guardian", async () => {
    ...
    for (let gate = 1; gate <= 10; gate++) {
      const data = JSON.parse(
        (await generateStoryPrompt({ gate })).content[0].text,
      );
      assert.equal(
        data.guardian,
        expected[gate - 1],
        `Gate ${gate} should route to ${expected[gate - 1]}`,
      );
    }
  });
  ...
  it("generateMagicAbility output is valid JSON parseable text", async () => {
    const result = await generateMagicAbility({
      element: "Fire",
      gateLevel: 5,
    });
    assert.doesNotThrow(() => JSON.parse(result.content[0].text));
...
describe("Error Handling — diagnoseBlock with extreme inputs", () => {
  ...
  it("symptoms with multiple simultaneous creatures still returns a primary match", async () => {
    const symptoms =
      "I feel like a fraud, am exhausted, keep comparing myself to others, procrastinating, and stuck on a blank page";
  ...
  it("gibberish input returns no-match response without throwing", async () => {
    const data = JSON.parse(
      (await diagnoseBlock("zzz aaa bbb ccc ddd")).content[0].text,
    );
    assert.ok(
      data.diagnosis !== undefined || data.generalRemedies !== undefined,
    );
  });
  ...
  it("context-only match (weak symptoms) still returns structured response", async () => {
    const data = JSON.parse(
      (
        await diagnoseBlock(
          "I feel bad",
          "I am so exhausted and drained every day",
        )
      ).content[0].text,
    );
    assert.ok(typeof data.diagnosis === "string");
  ...
  it("single keyword match still returns creature info", async () => {
    ...
    assert.ok(
      data.diagnosis.includes("Imposter Shade") ||
...
describe("Error Handling — validateCanon edge cases", () => {
  ...
  it("content with many guardian names is validated without throwing", async () => {
    const content =
      "Lyssandria, Leyla, Draconia, Maylinn, Alera, Lyria, Aiyami, Elara, Ino, Shinkami all gathered.";
  ...
  it('content type "general" works without adding type-specific suggestions', async () => {
    const data = JSON.parse(
      (await validateCanon("A neutral test.", "general")).content[0].text,
    );
    assert.ok(Array.isArray(data.suggestions));
    assert.equal(
      data.suggestions.length,
      0,
      "general type should have no specific suggestions",
    );
  });

  it("valid flag is boolean", async () => {
    const data = JSON.parse(
      (await validateCanon("Clean content here.", "story")).content[0].text,
    );
  ...
  it("issueCount matches issues array length", async () => {
    const data = JSON.parse(
      (await validateCanon("Nero is an evil villain.", "story")).content[0]
        .text,
    );
  ...
  it("issues array contains objects with type and message fields", async () => {
    const data = JSON.parse(
      (await validateCanon("Nero is evil.", "story")).content[0].text,
    );
  ...
  it("summary field is always a non-empty string", async () => {
    const data = JSON.parse(
      (await validateCanon("Any content.", "story")).content[0].text,
    );
...
describe("Error Handling — memory system edge cases", () => {
  ...
  it("recordLuminorConsulted is idempotent", () => {
    ...
    const session = memIndex.getOrCreateSession(sid);
    const count = session.luminorsConsulted.filter(
      (l) => l === "Valora",
    ).length;
  ...
  it("recordCreatureEncountered is idempotent", () => {
    ...
    const session = memIndex.getOrCreateSession(sid);
    const count = session.creaturesEncountered.filter(
      (c) => c === "Imposter Shade",
    ).length;
    assert.equal(count, 1);
  });
...
describe("Error Handling — council edge cases", () => {
  ...
  before(async () => {
    ({ conveneCouncil, luminorDebate } =
      await import("../dist/tools/council.js"));
  });

  it("conveneCouncil with empty support array still works", async () => {
    const data = JSON.parse(
      (await conveneCouncil("valora", [], "any topic")).content[0].text,
    );
  ...
  it("conveneCouncil with all valid luminors in support", async () => {
    ...
    const data = JSON.parse(
      (
        await conveneCouncil(
          "valora",
          ["serenith", "ignara", "verdana", "eloqua"],
          "everything",
        )
      ).content[0].text,
    );
    if ("error" in data) {
      ...
      assert.fail(
        `conveneCouncil with valid inputs returned error: ${data.error}`,
      );
    }
  ...
  it("conveneCouncil topic is reflected in lead perspective", async () => {
    ...
    const data = JSON.parse(
      (await conveneCouncil("ignara", [], topic)).content[0].text,
    );
    if (!("error" in data)) {
      assert.ok(
        data.lead.perspective.includes(topic),
        "Topic should appear in lead perspective",
      );
    }
  });
  ...
  it("luminorDebate positions array always has exactly 2 entries for valid slugs", async () => {
    const data = JSON.parse(
      (await luminorDebate("valora", "serenith", "time")).content[0].text,
    );
    if (!("error" in data)) {
...
describe("Error Handling — creation graph edge cases", () => {
  ...
  it("findPath returns null for disconnected nodes", () => {
    ...
    assert.ok(
      path === null || Array.isArray(path),
      "findPath should return null or array",
    );
  });
...
describe("Canon Compliance — Extended Solfeggio frequencies (MEMORY.md locked)", () => {
  ...
  it("frequencies parse to numbers that are all different", () => {
    ...
    assert.equal(
      unique.size,
      nums.length,
      "All frequencies should be numerically distinct",
    );
  });
  ...
  it("domain-guardian mapping is canonical", () => {
    ...
    assert.equal(
      uniqueGuardians.size,
      10,
      "Each domain should have a unique guardian",
    );
  });
});

// Actual discovery and resource reads are verified through the MCP SDK in
// canon.test.mjs, library-search.test.mjs and registration-composition.test.mjs.
// Test-local name arrays cannot establish which tools or resources are exposed.
...
describe("Library Search — keyword parsing and scoring logic", () => {
  it("short keywords (< 2 chars) are filtered out", async () => {
    ...
    assert.ok(
      data.keywords.every((k) => k.length >= 2),
      "All keywords should have length >= 2",
    );
  });
  ...
  it("limit=1 returns exactly 1 result when matches exist", async () => {
    ...
    assert.ok(
      data.results.length <= 1,
      `Expected <= 1 result for limit=1, got ${data.results.length}`,
    );
  });
});
