...
const CUSTOM_CANON = readFileSync(
  resolve(here, "../../world-pack/fixtures/custom-canon.md"),
  "utf8",
);
...
test("a forged layer:'canon' node is a blocker, not a blessing", async () => {
  ...
  assert.ok(
    claim,
    `expected canon.layer-claim, got ${body.findings.map((f) => f.ruleId).join(", ")}`,
  );
...
test("a clean pack passes with zero findings against the bundled canon", async () => {
  ...
  assert.equal(
    body.canon.sourceHash,
    pack.canon.sourceHash,
    "the bundled canon is the canon the golden pack was cleared against",
  );
});
...
test("path is read over stdio; bad inputs are tool errors, not crashes", async () => {
  ...
  const notJson = await call("worldpack_check", {
    path: resolve(here, "../../world-pack/fixtures/custom-canon.md"),
  });
...
test("a creator's own canonDocument enforces their locked names", async () => {
  ...
  const { body } = await call("worldpack_check", {
    pack,
    canonDocument: CUSTOM_CANON,
  });
  ...
  assert.ok(
    rules.includes("canon.binding-foreign"),
    "a foreign binding is reported, not enforced",
  );
...
test("worldpack_verify tells sealed, tampered, unsealed and canon-mismatch apart", async () => {
  assert.equal(
    (await call("worldpack_verify", { pack: golden() })).body.verdict,
    "sealed",
  );
  ...
  assert.notEqual(
    tampered.checks.digest.recomputed,
    tampered.checks.digest.declared,
  );

  const working = golden();
  delete working.digest;
  assert.equal(
    (await call("worldpack_verify", { pack: working })).body.verdict,
    "unsealed",
  );

  const foreign = (
    await call("worldpack_verify", {
      pack: golden(),
      canonDocument: CUSTOM_CANON,
    })
  ).body;
...
test("worldpack_rules covers exactly the rules the engine can emit", async () => {
  ...
  assert.deepEqual(
    body.rules.map((r) => r.ruleId).sort(),
  ...
  const one = (await call("worldpack_rules", { ruleId: "canon.layer-claim" }))
    .body;
  assert.equal(one.severity, "blocker");
  assert.equal(
    (await call("worldpack_rules", { ruleId: "canon.made-up" })).isError,
    true,
  );
});

test("the README tool table lists exactly what tools/list returns", async () => {
  const section = README.split("<!-- tools:start -->")[1]?.split(
    "<!-- tools:end -->",
  )[0];
  assert.ok(
    section,
    "README must mark its tool table with <!-- tools:start --> / <!-- tools:end -->",
  );
  const documented = [...section.matchAll(/^\|\s*`([a-z0-9_]+)`\s*\|/gm)]
    .map((m) => m[1])
    .sort();
