...
2. Read [the catalog](../../catalog.json). Select the closest job: Resonant for a producer, Crate for a sample collection, Fieldwork for a lab, Open model for a model release, Forma for a creative tool, Relay for an agent workflow, Session for a release listening room, Margin for a paper, Patch for a developer tool.
...
4. From the plugin root run `node scripts/build.mjs --out <new-absolute-output-directory>`. The destination must be empty. This builds the gallery, nine self-contained HTML starters, matching v0 Markdown briefs and registry bundles without installing packages or contacting providers. Never modify the installed plugin cache as a project workspace.
5. For v0, prefer the generated source bundle on a verified public URL, or attach the selected HTML and Markdown brief to a new v0 chat and ask for Next.js App Router + strict TypeScript. Use an available v0 tool only when its call is authorized. Otherwise deliver the files and prompt; do not claim a v0 generation or published template occurred.
6. For a local implementation, adapt the chosen source to existing components and design tokens. Use the common shell and the smallest matching composition; do not fork a complete application for every catalog entry. Introduce a provider, database or MCP server only when the user's real workflow requires it. Follow [architecture](../../references/architecture.md).
