...
describe('createArc', () => {
    it('creates a valid arc with required fields', () => {
        const arc = (0, index_1.createArc)({ type: 'character', creator: 'test-user' });
    ...
    it('creates unique IDs for each arc', () => {
        const a = (0, index_1.createArc)({ type: 'world', creator: 'u1' });
        const b = (0, index_1.createArc)({ type: 'world', creator: 'u1' });
        assert(a.id !== b.id, 'two arcs should have different IDs');
    });
    it('includes APL when spark is provided', () => {
        const arc = (0, index_1.createArc)({ type: 'story', creator: 'u1', spark: 'A dragon who paints' });
    ...
    it('includes APL when palette is provided', () => {
        const arc = (0, index_1.createArc)({ type: 'image', creator: 'u1', palette: 'forge' });
    ...
    it('includes APL when sharpen is provided', () => {
        const arc = (0, index_1.createArc)({ type: 'music', creator: 'u1', sharpen: ['generic', 'cliche'] });
    ...
    it('includes tags when provided', () => {
        const arc = (0, index_1.createArc)({ type: 'world', creator: 'u1', tags: ['fantasy', 'dark'] });
        assertDeepEqual(arc.tags, ['fantasy', 'dark'], 'tags should match');
    });
    it('includes gate when provided', () => {
        const arc = (0, index_1.createArc)({ type: 'character', creator: 'u1', gate: 5 });
        assertEqual(arc.gate, 5, 'gate should be 5');
    });
    it('includes element when provided', () => {
        const arc = (0, index_1.createArc)({ type: 'creature', creator: 'u1', element: 'fire' });
    ...
    it('supports all ArcType values', () => {
        ...
        for (const t of types) {
            const arc = (0, index_1.createArc)({ type: t, creator: 'u1' });
...
describe('advanceStage', () => {
    it('advances from potential to manifestation', () => {
        const arc = (0, index_1.createArc)({ type: 'story', creator: 'u1' });
        const advanced = (0, index_1.advanceStage)(arc, { at: new Date().toISOString(), model: 'claude-opus-4.6' });
        assertEqual(advanced.stage, 'manifestation', 'stage should be manifestation');
    });
    it('advances from manifestation to experience', () => {
        let arc = (0, index_1.createArc)({ type: 'story', creator: 'u1' });
        arc = (0, index_1.advanceStage)(arc, { at: new Date().toISOString() });
        const advanced = (0, index_1.advanceStage)(arc, { at: new Date().toISOString(), note: 'shared' });
    ...
    it('advances from experience to dissolution', () => {
        ...
        arc = (0, index_1.advanceStage)(arc, { at: new Date().toISOString() });
        arc = (0, index_1.advanceStage)(arc, { at: new Date().toISOString() });
        const advanced = (0, index_1.advanceStage)(arc, { at: new Date().toISOString() });
    ...
    it('cycles from evolved back to potential', () => {
        let arc = (0, index_1.createArc)({ type: 'story', creator: 'u1' });
        arc = (0, index_1.advanceStage)(arc, { at: new Date().toISOString() }); // manifestation
        arc = (0, index_1.advanceStage)(arc, { at: new Date().toISOString() }); // experience
        arc = (0, index_1.advanceStage)(arc, { at: new Date().toISOString() }); // dissolution
        arc = (0, index_1.advanceStage)(arc, { at: new Date().toISOString() }); // evolved
        const cycled = (0, index_1.advanceStage)(arc, { at: new Date().toISOString() }); // back to potential
    ...
    it('full cycle: potential -> manifestation -> experience -> dissolution -> evolved -> potential', () => {
        ...
        let arc = (0, index_1.createArc)({ type: 'world', creator: 'u1' });
    ...
    it('appends history entry with correct stage', () => {
        ...
        const advanced = (0, index_1.advanceStage)(arc, entry);
    ...
    it('does not mutate the original arc', () => {
        ...
        (0, index_1.advanceStage)(arc, { at: new Date().toISOString() });
...
describe('bond', () => {
    it('adds a bond to an arc with no existing bonds', () => {
        const arc = (0, index_1.createArc)({ type: 'character', creator: 'u1' });
        const bonded = (0, index_1.bond)(arc, { target: 'arc_world1', relation: 'inhabits' });
    ...
    it('adds a bond with a note', () => {
        const arc = (0, index_1.createArc)({ type: 'story', creator: 'u1' });
        const bonded = (0, index_1.bond)(arc, { target: 'arc_music1', relation: 'soundtrack', note: 'theme song' });
        assertEqual(bonded.bonds[0].note, 'theme song', 'note should match');
    });
    it('adds multiple bonds', () => {
        let arc = (0, index_1.createArc)({ type: 'character', creator: 'u1' });
        arc = (0, index_1.bond)(arc, { target: 'arc_world1', relation: 'inhabits' });
        arc = (0, index_1.bond)(arc, { target: 'arc_char2', relation: 'opposes' });
        assertEqual(arc.bonds.length, 2, 'should have 2 bonds');
    });
    it('prevents duplicate bonds (same target + relation)', () => {
        let arc = (0, index_1.createArc)({ type: 'character', creator: 'u1' });
        arc = (0, index_1.bond)(arc, { target: 'arc_world1', relation: 'inhabits' });
        const result = (0, index_1.bond)(arc, { target: 'arc_world1', relation: 'inhabits' });
        assertEqual(result.bonds.length, 1, 'duplicate bond should not be added');
    });
    it('allows same target with different relation', () => {
        let arc = (0, index_1.createArc)({ type: 'character', creator: 'u1' });
        arc = (0, index_1.bond)(arc, { target: 'arc_world1', relation: 'inhabits' });
        arc = (0, index_1.bond)(arc, { target: 'arc_world1', relation: 'creates' });
    ...
    it('allows same relation with different target', () => {
        let arc = (0, index_1.createArc)({ type: 'story', creator: 'u1' });
        arc = (0, index_1.bond)(arc, { target: 'arc_img1', relation: 'illustrates' });
        arc = (0, index_1.bond)(arc, { target: 'arc_img2', relation: 'illustrates' });
        assertEqual(arc.bonds.length, 2, 'same relation different target should be allowed');
    });
    it('sets updated timestamp', () => {
        const arc = (0, index_1.createArc)({ type: 'story', creator: 'u1' });
        const bonded = (0, index_1.bond)(arc, { target: 'arc_x', relation: 'forks' });
        assert(bonded.updated !== undefined, 'updated should be set after bonding');
    });
    it('returns same arc reference for duplicate bond', () => {
        let arc = (0, index_1.createArc)({ type: 'story', creator: 'u1' });
        arc = (0, index_1.bond)(arc, { target: 'arc_x', relation: 'forks' });
        const result = (0, index_1.bond)(arc, { target: 'arc_x', relation: 'forks' });
        assert(result === arc, 'should return same arc reference for duplicate');
    });
    it('does not mutate the original arc', () => {
        const arc = (0, index_1.createArc)({ type: 'story', creator: 'u1' });
        (0, index_1.bond)(arc, { target: 'arc_x', relation: 'forks' });
...
describe('toAgentContext', () => {
    ...
    it('includes SPARK from APL', () => {
        const arc = (0, index_1.createArc)({ type: 'story', creator: 'u1', spark: 'A lonely king' });
        const ctx = (0, index_1.toAgentContext)(arc);
        assertIncludes(ctx, 'SPARK: A lonely king', 'should include spark');
    });
    it('includes PALETTE in uppercase', () => {
        const arc = (0, index_1.createArc)({ type: 'image', creator: 'u1', palette: 'tide' });
        const ctx = (0, index_1.toAgentContext)(arc);
        assertIncludes(ctx, 'PALETTE: TIDE', 'should include palette in uppercase');
    });
    it('includes SHARPEN with NOT prefix', () => {
        const arc = (0, index_1.createArc)({ type: 'story', creator: 'u1', sharpen: ['generic', 'cliche'] });
        const ctx = (0, index_1.toAgentContext)(arc);
    ...
    it('includes bonds when present', () => {
        let arc = (0, index_1.createArc)({ type: 'character', creator: 'u1' });
        arc = (0, index_1.bond)(arc, { target: 'arc_world1', relation: 'inhabits' });
        arc = (0, index_1.bond)(arc, { target: 'arc_music1', relation: 'soundtrack' });
        const ctx = (0, index_1.toAgentContext)(arc);
    ...
    it('includes last model from history', () => {
        const arc = (0, index_1.createArc)({ type: 'story', creator: 'u1' });
        const advanced = (0, index_1.advanceStage)(arc, { at: new Date().toISOString(), model: 'claude-opus-4.6' });
        const ctx = (0, index_1.toAgentContext)(advanced);
...
describe('serialize', () => {
    it('produces YAML frontmatter with --- delimiters', () => {
        const arc = (0, index_1.createArc)({ type: 'world', creator: 'u1' });
        const out = (0, index_1.serialize)(arc);
        assert(out.startsWith('---\n'), 'should start with ---');
        assert(out.includes('\n---\n'), 'should have closing ---');
    });
    it('includes all required fields', () => {
        const arc = (0, index_1.createArc)({ type: 'character', creator: 'frankx' });
        const out = (0, index_1.serialize)(arc);
    ...
    it('serializes gate as number', () => {
        const arc = (0, index_1.createArc)({ type: 'world', creator: 'u1', gate: 7 });
        const out = (0, index_1.serialize)(arc);
        assertIncludes(out, 'gate: 7', 'should include gate as number');
    });
    it('serializes APL block', () => {
        const arc = (0, index_1.createArc)({
    ...
    it('serializes history array of objects', () => {
        const arc = (0, index_1.createArc)({ type: 'story', creator: 'u1', spark: 'test' });
        const out = (0, index_1.serialize)(arc);
...
---
`;
        const arc = (0, index_1.parse)(input);
...
`;
    ...
    it('throws on invalid file without frontmatter', () => {
        let threw = false;
        try {
            (0, index_1.parse)('no frontmatter here');
        }
        catch (e) {
            threw = true;
            assert(e instanceof Error, 'should throw Error');
            assertIncludes(e.message, 'no frontmatter', 'error should mention frontmatter');
        }
...
describe('round-trip (serialize -> parse)', () => {
    it('round-trips a minimal arc', () => {
        const original = (0, index_1.createArc)({ type: 'world', creator: 'u1' });
        const serialized = (0, index_1.serialize)(original);
        const parsed = (0, index_1.parse)(serialized);
    ...
    it('round-trips an arc with APL', () => {
        const original = (0, index_1.createArc)({
            type: 'story', creator: 'u1',
            spark: 'A dragon who paints', palette: 'forge', sharpen: ['generic', 'cliche'],
        });
        const parsed = (0, index_1.parse)((0, index_1.serialize)(original));
    ...
    it('round-trips tags', () => {
        const original = (0, index_1.createArc)({ type: 'world', creator: 'u1', tags: ['fantasy', 'dark', 'epic'] });
        const parsed = (0, index_1.parse)((0, index_1.serialize)(original));
        assertDeepEqual(parsed.tags, ['fantasy', 'dark', 'epic'], 'tags should round-trip');
    });
    it('round-trips gate and element', () => {
        const original = (0, index_1.createArc)({ type: 'creature', creator: 'u1', gate: 7, element: 'fire' });
        const parsed = (0, index_1.parse)((0, index_1.serialize)(original));
    ...
    it('round-trips body content', () => {
        const original = (0, index_1.createArc)({ type: 'story', creator: 'u1' });
    ...
    it('round-trips an arc after advancement with bonds', () => {
        let original = (0, index_1.createArc)({
            type: 'character', creator: 'frankx',
            spark: 'A warrior poet', palette: 'drift',
            tags: ['warrior', 'poet'], gate: 3, element: 'wind',
        });
        original = (0, index_1.advanceStage)(original, { at: '2026-03-17T01:00:00Z', model: 'claude-opus-4.6', note: 'generated' });
        original = (0, index_1.bond)(original, { target: 'arc_world1', relation: 'inhabits', note: 'home world' });
...
describe('validate', () => {
    it('validates a correct arc as valid', () => {
        const arc = (0, index_1.createArc)({ type: 'world', creator: 'u1' });
        const result = (0, index_1.validate)(arc);
    ...
    it('accepts valid gates 1-10', () => {
        for (let g = 1; g <= 10; g++) {
            const arc = (0, index_1.createArc)({ type: 'world', creator: 'u1', gate: g });
            const result = (0, index_1.validate)(arc);
            assertEqual(result.valid, true, `gate ${g} should be valid`);
        }
    });
    it('warns on empty spark in APL block', () => {
        const arc = (0, index_1.createArc)({ type: 'story', creator: 'u1', palette: 'forge' });
...
describe('edge cases', () => {
    it('handles arc with all optional fields populated', () => {
        let arc = (0, index_1.createArc)({
            type: 'character', creator: 'frankx',
            spark: 'A lonely king', palette: 'tide',
            sharpen: ['noble', 'dramatic'], tags: ['king', 'grief'],
            gate: 2, element: 'water',
        });
        arc = (0, index_1.advanceStage)(arc, { at: '2026-03-17T01:00:00Z', model: 'claude-opus-4.6', quality: 92 });
        arc = (0, index_1.bond)(arc, { target: 'arc_music1', relation: 'soundtrack', note: 'theme' });
        ...
        const serialized = (0, index_1.serialize)(arc);
    ...
    it('handles empty tags array gracefully', () => {
        const arc = (0, index_1.createArc)({ type: 'world', creator: 'u1', tags: [] });
