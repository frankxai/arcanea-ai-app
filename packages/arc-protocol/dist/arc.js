...
exports.bond = bond;
exports.toAgentContext = toAgentContext;
...
function createArc(options) {
    ...
    if (options.tags)
        arc.tags = options.tags;
    if (options.gate !== undefined)
        arc.gate = options.gate;
    if (options.element)
        arc.element = options.element;
    return arc;
}
// ── Stage Advancement ────────────────────────────────────────────────────────
const STAGE_ORDER = ['potential', 'manifestation', 'experience', 'dissolution', 'evolved'];
function advanceStage(arc, entry) {
...
function toAgentContext(arc) {
    ...
    if (arc.apl) {
        if (arc.apl.spark)
            lines.push(`SPARK: ${arc.apl.spark}`);
        if (arc.apl.palette)
            lines.push(`PALETTE: ${arc.apl.palette.toUpperCase()}`);
    ...
    if (arc.agent) {
        ...
        if (arc.agent.next_step)
            lines.push(`NEXT: ${arc.agent.next_step}`);
...
function parse(content) {
    ...
    if (!fmMatch)
        throw new Error('Invalid .arc file: no frontmatter found');
    const yaml = fmMatch[1];
    const body = content.slice(fmMatch[0].length).trim() || undefined;
    const arc = fromYaml(yaml);
    if (body)
        arc.body = body;
    return arc;
}
function validate(arc) {
    const errors = [];
    const warnings = [];
    if (arc.arc !== '1.0')
        errors.push(`Unknown arc version: ${arc.arc}`);
    if (!arc.id)
    ...
    for (const [key, value] of Object.entries(obj)) {
        ...
        if (typeof value === 'string') {
            ...
            if (typeof value[0] === 'string') {
                ...
                value.forEach(v => {
                    result += `${pad}  - `;
                    const inner = toYaml(v, indent + 2).trim();
            ...
            if (children.length > 0 && children[0].trim().startsWith('-')) {
                ...
                for (const child of children) {
                    ...
                    if (trimmed.startsWith('- ')) {
                        if (item)
                            items.push(item.startsWith('"') ? item.slice(1, -1) : item);
