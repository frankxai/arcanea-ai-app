...
let channelGuardian: GuardianName | undefined;
if (guardianFlag !== -1 && args[guardianFlag + 1]) {
  channelGuardian = args[guardianFlag + 1] as GuardianName;
}

// Filter out our custom flags from args passed to claude
const claudePassthroughArgs = args.filter((arg, i) => {
  if (arg === '--compact' || arg === '--no-ios') return false;
  if (arg === '--guardian' || arg === '--agents') return false;
  if (i > 0 && args[i - 1] === '--guardian') return false;
  if (arg === '--detect') return false;
  if (i > 0 && args[i - 1] === '--detect') return false;
  return true;
});
...
let iosPromptPath: string | null = null;
