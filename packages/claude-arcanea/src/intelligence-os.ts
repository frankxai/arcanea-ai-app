...
import type { GuardianName, GateName, Guardian } from '@arcanea/core';

/**
 * Extended Guardian properties that exist at runtime in the constants
 * but may not be present in older compiled type declarations.
 * This bridge type ensures we can access the full Guardian data.
 */
...
function ext(g: Guardian): GuardianExt {
  return g as GuardianExt;
}
 ...
 */
export function detectTaskRoute(input: string): {
  domain: string;
  guardian: GuardianName;
  suggestedModel: 'opus' | 'sonnet' | 'haiku';
  description: string;
...
function generateProactiveBehavior(): string {
...
function generateLuminaVoice(): string {
...
function generateHierarchySection(): string {
...
function generateGatesProgression(): string {
  ...
  const rankRows = MAGIC_RANKS.map(r =>
    `| ${r.gatesRequired[0]}-${r.gatesRequired[1]} | ${r.rank.charAt(0).toUpperCase() + r.rank.slice(1)} | ${r.description} |`
  ).join('\n');
...
function generateCreativeDomainsSection(): string {
...
function generateGuardianRouting(): string {
...
const DEFAULT_OPTIONS: IntelligenceOSOptions = {
 ...
 */
export function buildIntelligenceOSPrompt(options: IntelligenceOSOptions = {}): string {
 ...
 */
export function buildCompactPrompt(channelGuardian?: GuardianName): string {
