  ...
  return (
    <div className="flex items-center gap-0.5" aria-label={`${rating} out of 5 stars`}>
      {Array.from({ length: 5 }).map((_, i) => (
        <svg
          key={i}
          className={`w-4 h-4 ${i < full ? "text-[var(--arc-brand-arcanean-gold)]" : i === full && half ? "text-[var(--arc-brand-arcanean-gold)]/60" : "text-white/15"}`}
          viewBox="0 0 24 24"
          fill="currentColor"
          aria-hidden="true"
        >
...
export function StreamingCursor() {
  return (
    <span aria-hidden="true">
      <m.span
        className="inline-block w-0.5 h-4 bg-[var(--arc-brand-atlantean-teal)] ml-0.5 align-middle"
          ...
          {agent.examplePrompts.map((prompt, i) => (
            <li key={i}>
              <button
                ...
                className="w-full text-left text-sm text-white/50 hover:text-white/90 bg-white/[0.02] hover:bg-white/[0.06] border border-white/[0.06] hover:border-white/[0.12] rounded-xl px-3 py-2.5 transition-all leading-snug focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--arc-brand-atlantean-teal)]/60"
