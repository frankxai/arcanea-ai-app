 ...
 * Atlantean Teal pulse on live signals; Gold for success / merged states.
...
const ACCENT_STYLES: Record<
  NonNullable<SignalCardProps["accent"]>,
  { dot: string; glow: string; text: string }
> = {
  teal: {
    dot: "var(--arc-brand-atlantean-teal)",
    glow: "rgba(0,188,212,0.18)",
    text: "var(--arc-brand-atlantean-teal)",
  },
  gold: {
    dot: "var(--arc-brand-arcanean-gold)",
...
export function SignalCard({
  ...
  return (
    <section
      ...
      style={{
        boxShadow: accent !== "neutral" ? `0 0 60px ${a.glow}` : undefined,
      }}
    >
      <header className="flex items-center gap-2 mb-4">
        <span
          ...
          style={{
            ...
            boxShadow:
              live && accent !== "neutral" ? `0 0 10px ${a.dot}` : "none",
          }}
          aria-hidden
        />
        <h2
...
export function BriefingCard() {
  ...
  return (
    <SignalCard
      ...
      live={!loading}
      span="wide"
      cta={{ href: "/api/voice/briefing", label: "Raw" }}
    >
      {loading ? (
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <Stat
              ...
              accent={
                memUsedPct !== null && memUsedPct > 88
                  ? "rose"
                  : memUsedPct !== null && memUsedPct > 70
                    ? "gold"
                    : undefined
              }
            />
            <Stat
              label="Plans"
              value={
                s?.planningFiles !== undefined ? String(s.planningFiles) : "—"
              }
            />
          </div>
          {data.brief && (
          ...
          >
            {data.source === "local"
              ? "local — full filesystem awareness"
              : data.source === "remote"
                ? "remote — public surface only"
                : "partial"}
          </p>
        </div>
      )}
...
export function HeroPanel() {
  return (
    <section className="relative md:col-span-4 rounded-2xl overflow-hidden border border-white/[0.06] bg-gradient-to-br from-[var(--arc-brand-atlantean-teal)]/[0.10] via-[var(--arc-brand-atlantean-teal)]/[0.04] to-transparent backdrop-blur-md p-8 min-h-[260px] flex flex-col justify-between">
      <div
        aria-hidden
        className="absolute -top-24 -right-24 w-72 h-72 rounded-full blur-3xl opacity-50"
        style={{
          background:
            "radial-gradient(circle, rgba(0,188,212,0.35) 0%, transparent 70%)",
        }}
      />
      <div className="relative">
        <p
          className="text-[10px] tracking-[0.36em] uppercase text-[var(--arc-brand-atlantean-teal)]/70 mb-3"
          style={{ fontFamily: "var(--font-mono, var(--font-display))" }}
        >
          Voice Operator
        </p>
        ...
        >
          Jarvis is on the room. Six live tools. Sir-cadence prompt. Speak when
          ready.
        </p>
      </div>
...
export function PRsCard() {
  ...
  useEffect(() => {
    let cancelled = false;
    callTool<{ count: number; prs: PrItem[]; error?: string }>(
      "list_open_prs",
    ).then((data) => {
      if (cancelled) return;
      if (!data) {
  ...
  return (
    <SignalCard
      label="Open PRs"
      accent="gold"
      live={prs !== null && prs.length > 0}
      span="normal"
      cta={{
        href: "https://github.com/frankxai/arcanea-ai-app/pulls",
        label: "GitHub",
      }}
    >
      {prs === null && !error ? (
        ...
        <p
          className="text-white/45 text-sm italic"
          style={{
            fontFamily: "var(--font-editorial), var(--font-serif), serif",
          }}
        >
        ...
        <ol className="space-y-2.5">
          {prs!.slice(0, 6).map((pr) => (
            <li key={pr.number} className="flex items-baseline gap-3 group">
              <span
                className="shrink-0 text-[11px] text-[var(--arc-brand-arcanean-gold)]/80"
                style={{
...
export function CommitsCard() {
  ...
  useEffect(() => {
    let cancelled = false;
    callTool<{ count: number; commits: CommitItem[] }>("git_today").then(
      (res) => {
        if (cancelled) return;
        setData(res?.commits ?? []);
      },
        ...
        <ol className="space-y-2.5">
          {data.slice(0, 7).map((c) => (
            <li key={c.hash} className="flex items-baseline gap-3">
              <span
                className="shrink-0 text-[11px] text-[var(--arc-brand-atlantean-teal)]/80"
                style={{
                  fontFamily: "var(--font-mono, monospace)",
                  letterSpacing: "0.04em",
                }}
...
const PERSONA_ROSTER: Array<{
  ...
  },
  {
    id: "draconia",
    name: "Draconia",
    tag: "Guardian of Fire",
    color: "var(--arc-fire)",
  },
  {
    id: "lyria",
    name: "Lyria",
    tag: "Guardian of Sight",
    color: "var(--arc-void)",
  },
  {
    id: "alera",
    name: "Alera",
    tag: "Guardian of Voice",
    color: "var(--arc-brand-atlantean-teal)",
  },
  {
    id: "shinkami",
            ...
            >
              <span
                ...
                style={{
                  ...
                  boxShadow: `0 0 12px color-mix(in srgb, ${p.color} 40%, transparent)`,
                }}
                aria-hidden
              />
              <span
