...
import { createCompanionFromArchetype } from '@/lib/companions/companion-spec';
...
function PersonalityPill({ trait, selected, onClick }: { trait: string; selected: boolean; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
export default function ForgeCompanionPage() {
  ...
  // Build the companion spec when forge completes
  const companionSpec =
    forgeComplete && selectedArchetype
      ? createCompanionFromArchetype(
          selectedArchetype,
          customName.trim() || selectedArchetype.name,
          selectedElement,
          selectedPersonality,
        )
      : null;
  ...
  const handleCopySpec = useCallback(() => {
    if (!companionSpec) return;
    navigator.clipboard.writeText(JSON.stringify(companionSpec, null, 2));
    setCopied(true);
      ...
      <div className="min-h-screen bg-[#09090b] text-white">
        ...
        <section className="relative overflow-hidden px-6 pb-12 pt-24 text-center md:pt-32">
          ...
          >
            ...
            <h1 className="font-display text-4xl font-bold tracking-tight md:text-5xl">
              Call Forth a Creature
            </h1>
            <p className="mx-auto mt-4 max-w-lg text-base text-white/45">
              Choose an archetype from the Five Elements. Name it. Give it
              personality. It becomes your visual identity on Arcanea.
            </p>
          </m.div>
        </section>
        ...
        <section className="mx-auto max-w-6xl px-6 pb-16">
          ...
          >
            ...
            {/* Element filter */}
            <div className="mb-8 flex flex-wrap gap-2" role="tablist" aria-label="Filter by element">
              {ELEMENTS.map((el) => {
                ...
                {filteredArchetypes.map((archetype) => {
                  ...
                  return (
                    <m.button
                      ...
                      aria-label={`${archetype.name} — ${archetype.element} archetype`}
                      ...
                      style={
                        isSelected
                          ? { borderColor: `${archetype.baseColor}60`, boxShadow: `0 0 24px ${archetype.glowColor}, 0 0 48px ${archetype.glowColor}` }
                          : isRare
                      ...
                      <div className="relative z-10 p-5">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {archetype.personality.map((trait) => (
                            <span key={trait} className="rounded-full border border-white/[0.06] bg-white/[0.03] px-2 py-0.5 text-[10px] text-white/40">
                              {trait}
                            </span>
                          ))}
                        </div>
          ...
          {selectedArchetype && (
            ...
            >
              <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-6 backdrop-blur-md md:p-8">
                <h2 className="mb-1 font-display text-lg font-semibold tracking-tight text-white/80">Step 2</h2>
                <p className="mb-8 text-sm text-white/40">Customize your creature</p>

                {/* Selected preview */}
                <div className="mb-8 flex items-start gap-5">
                  <div
                    className="flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-xl text-3xl"
                    style={{ background: `${selectedArchetype.baseColor}15`, boxShadow: `0 0 20px ${selectedArchetype.glowColor}` }}
                  >
                    {selectedArchetype.icon}
                  </div>
                  <div>
                    <h3 className="font-display text-base font-semibold text-white/90">{selectedArchetype.name}</h3>
                    <p className="mt-1 text-sm leading-relaxed text-white/40">{selectedArchetype.description}</p>
                  </div>
                </div>

                {/* Name */}
                <div className="mb-8">
                  <label htmlFor="companion-name" className="mb-2 block text-xs font-medium uppercase tracking-wider text-white/40">
                    Name your creature
                  </label>
                  <input
                    id="companion-name"
                ...
                {/* Element */}
                <div className="mb-8">
                  <p className="mb-3 text-xs font-medium uppercase tracking-wider text-white/40">Element</p>
                    ...
                    {ELEMENTS.filter((e) => e.key !== 'All').map((el) => {
                      ...
                      return (
                        <button
                          ...
                          style={isActive ? { borderColor: `${el.color}50`, boxShadow: `0 0 16px ${el.color}20` } : undefined}
                        >
                ...
                {/* Personality */}
                <div>
                  <p className="mb-1 text-xs font-medium uppercase tracking-wider text-white/40">Personality Traits</p>
                  <p className="mb-3 text-[11px] text-white/25">Select up to 5 traits</p>
                  <div className="flex flex-wrap gap-2">
          ...
          {selectedArchetype && customName.trim().length > 0 && (
            ...
            >
              <div className="text-center">
                <h2 className="mb-1 font-display text-lg font-semibold tracking-tight text-white/80">Step 3</h2>
                <p className="mb-8 text-sm text-white/40">Summon your creature</p>
                    ...
                    {isForging ? (
                      <>
                        <div
                          ...
                          style={{ background: `linear-gradient(90deg, transparent, ${activeColor}15, transparent)`, animation: 'shimmer 1.5s ease-in-out infinite' }}
                        />
                        <div className="relative flex items-center gap-3">
                          ...
                          />
                          <span className="text-white/70">Summoning...</span>
                        </div>
                      </>
                    ) : (
                      <>
                ...
                <AnimatePresence>
                  {forgeComplete && selectedArchetype && companionSpec && (
                    <m.div
                    ...
                    >
                      {/* Image placeholder */}
                      <div className={`relative flex h-48 items-center justify-center bg-gradient-to-br ${ELEMENT_GRADIENTS[activeElement]}`}>
                      ...
                      <div className="p-6 text-left">
                        <div className="mb-3 flex items-center justify-between">
                          <h3 className="font-display text-xl font-bold tracking-tight">{customName.trim()}</h3>
                          <div className="flex items-center gap-1.5">
                            {(() => {
                              const Icon = ELEMENT_ICONS[activeElement];
                              return <Icon size={14} style={{ color: activeColor }} />;
                            })()}
                            <span className="text-xs font-medium" style={{ color: activeColor }}>{activeElement}</span>
                          </div>
                        </div>

                        <p className="mb-4 text-xs text-white/30">
                          Born from the <span className="text-white/50">{selectedArchetype.name}</span> archetype
                        </p>
                        ...
                        <div className="mb-4 flex flex-wrap gap-1.5">
                          {selectedPersonality.map((trait) => (
                            <span
                              ...
                              style={{ borderColor: `${activeColor}30`, color: `${activeColor}cc`, background: `${activeColor}10` }}
                            >
                              {trait}
                            </span>
                          ))}
                        ...
                        <div className="flex items-center justify-between border-t border-white/[0.06] pt-4">
                          ...
                          <button
                            ...
                            className="flex items-center gap-1.5 rounded-lg border border-white/[0.08] bg-white/[0.03] px-2.5 py-1 text-[11px] text-white/40 transition-colors hover:border-white/15 hover:text-white/60"
                          >
                            {copied ? (
                              <>
