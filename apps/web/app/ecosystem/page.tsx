...
const LAYERS = [
  ...
  },
  {
    ...
    description:
      "Multi-model orchestration, persistent memory, agent coordination, and repo-level context.",
    stats: [
      "SIS substrate",
      "MCP adapters",
...
const GROUP_ORDER: PublicRepoGroup[] = [
  "core",
  "intelligence",
  "tools",
  "protocol",
...
const REPO_GROUPS: { category: string; accent: string; repos: PublicRepo[] }[] =
...
const PACKAGE_DESCRIPTIONS: Record<string, string> = {
  ...
  "@arcanea/aios":
    "AI operating-system primitives referenced by the OSS framework",
  ...
  "@arcanea/mcp":
    "Historical MCP package reference; repo URL needs verification",
  "@arcanea/memory-mcp":
    "Historical memory package reference; likely superseded by SIS",
  "@claude-flow/cli":
    "Upstream orchestration CLI consumed as an external runtime",
};

const PACKAGES = PUBLIC_PACKAGE_NAMES.map((name) => ({
  name,
  desc:
    PACKAGE_DESCRIPTIONS[name] ?? "Package reference from the repo registry",
}));
...
const FLOW_STEPS = [
  ...
  {
    label: "AI APIs",
    sub: "Gemini, Claude, Grok",
    color: "var(--arc-brand-cosmic-blue)",
  },
  {
    label: "SIS",
    sub: "Memory and context substrate",
    color: "var(--arc-void)",
  },
  {
    label: "oh-my-arcanea",
    sub: "Manual harness sync",
    color: "var(--arc-brand-cosmic-blue)",
  },
  {
    label: "GitHub",
    sub: `${PUBLIC_REPO_SUMMARY.public} public repos`,
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-20">
          ...
          <div className="relative rounded-3xl overflow-hidden bg-white/[0.03] backdrop-blur-sm border border-white/[0.06] px-8 py-16 sm:px-14 sm:py-20">
            ...
            <div className="relative max-w-3xl">
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-display font-bold mb-6 leading-tight tracking-tight">
                <SplitText
                  as="span"
                  text={`${PUBLIC_REPO_SUMMARY.public} public repos.`}
                  className="bg-gradient-to-r from-[var(--arc-brand-atlantean-teal)] to-[var(--arc-brand-atlantean-teal)] bg-clip-text text-transparent"
                  delay={0.1}
                  stagger={0.04}
                />{" "}
                <SplitText
              ...
              <p className="text-lg text-white/50 leading-relaxed max-w-2xl mb-10">
                {PUBLIC_REPO_SUMMARY.active} active Arcanea repos are tracked in
                the registry.
                {` ${PUBLIC_REPO_SUMMARY.public}`} are public on GitHub today,{" "}
                {PUBLIC_REPO_SUMMARY.private} is private, and{" "}
                {PUBLIC_REPO_SUMMARY.unresolved} historical entries need
        ...
        <section className="mb-20" aria-labelledby="map-heading">
          <div className="mb-8">
            ...
            <p className="text-white/40 text-sm mt-1 max-w-2xl">
              Three views into the same 35-node graph:{" "}
              <span className="text-white/60">Layered</span> (substrate →
              product → surface),{" "}
              <span className="text-white/60">Ten Gates</span> (canonical
              mythology arrangement), and{" "}
              <span className="text-white/60">Arc ⊕ Nea</span> (sovereign /
              specialization hemispheres). Filter by status, hemisphere, or
              gate. Click any node to see what it consumes, who consumes it, and
              where to reach it.
            </p>
          </div>
        ...
        <section className="mb-20" aria-labelledby="arch-heading">
          <div className="mb-10">
            ...
            <p className="text-white/40 text-sm mt-1">
              Product, intelligence, and open source — each layer powers the
              next.
            </p>
          </div>
        ...
        <section className="mb-20" aria-labelledby="repos-heading">
          <div className="mb-10">
            ...
            <p className="text-white/40 text-sm mt-1 max-w-xl">
              Registry-derived map from{" "}
              <code className="font-mono text-white/45">
                .arcanea/config/repos.json
              </code>
              . Public links resolve; private and unresolved repos are labeled.
            </p>
          </div>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                  {group.repos.map((repo) => (
                    <LiquidGlass
                      ...
                      intensity="subtle"
                      tint={group.accent}
                      className="rounded-xl border border-white/[0.06] hover:border-white/[0.14] transition-colors"
                      noise={false}
                    >
                      {repo.url ? (
                        <a
                          href={repo.url}
                          target={
                            repo.url.startsWith("http") ? "_blank" : undefined
                          }
                          rel={
                          ...
                          className="group/card block relative p-5"
                        >
                          <div className="flex items-start justify-between mb-2">
                            <span className="font-mono text-xs text-white font-medium group-hover/card:text-[var(--arc-brand-atlantean-teal)] transition-colors truncate mr-2">
                              {repo.name}
                            </span>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <span
                              ...
                              style={{
                                color: STATUS_COLORS[repo.status],
                                backgroundColor: `${STATUS_COLORS[repo.status]}10`,
                              }}
                            >
                              {repo.status}
                            </span>
                        ...
                        <div className="relative p-5">
                          <div className="flex items-start justify-between mb-2">
                            <span className="font-mono text-xs text-white font-medium truncate mr-2">
                              {repo.name}
                            </span>
                          </div>
                          <p className="text-[11px] text-white/35 leading-relaxed mb-3">
        ...
        <section className="mb-20" aria-labelledby="packages-heading">
          <div className="mb-10">
            ...
            <p className="text-white/40 text-sm mt-1">
              Package references declared by the tracked repos. Historical
              packages stay labeled until their repo links are verified.
            </p>
          </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {PACKAGES.map((pkg) => (
              <div
                key={pkg.name}
                className="relative rounded-xl bg-white/[0.03] border border-white/[0.06] hover:border-white/[0.12] transition-all p-5"
              >
                <div className="flex items-center gap-2 mb-2">
                  <IconPackage className="w-3.5 h-3.5 text-[var(--arc-brand-arcanean-gold)]/60" />
                  <span className="font-mono text-xs text-[var(--arc-brand-arcanean-gold)] font-medium">
                    {pkg.name}
                  </span>
                </div>
                <p className="text-[11px] text-white/35 leading-relaxed mb-3">
        ...
        <section className="mb-20" aria-labelledby="flow-heading">
          <div className="mb-10">
            ...
            <p className="text-white/40 text-sm mt-1">
              Every request flows through the full intelligence stack.
            </p>
          </div>
              ...
              {FLOW_STEPS.map((step, i) => (
                <div key={step.label} className="flex items-center">
                  ...
                  {i < FLOW_STEPS.length - 1 && (
                    <div
                      className="flex-1 min-w-[20px] h-px mx-1"
                      style={{
                        background: `linear-gradient(to right, ${step.color}40, ${FLOW_STEPS[i + 1].color}40)`,
                      }}
                    />
                  )}
                </div>
          ...
          <div className="relative rounded-3xl overflow-hidden bg-white/[0.02] border border-white/[0.06] px-8 py-14 sm:px-14">
            ...
            <div className="relative max-w-2xl mx-auto text-center">
              ...
              <p className="text-white/40 text-sm mb-8 max-w-md mx-auto">
                Install the Arcanea harness from the public framework repo.
                Skills and agents are evolving; the registry marks what is
                active, beta, or unresolved.
              </p>

              {/* Install command */}
              <div className="inline-flex items-center gap-3 bg-white/[0.04] border border-white/[0.08] rounded-xl px-6 py-3 mb-10">
                <span className="text-[var(--arc-brand-atlantean-teal)] font-mono text-sm">
                  $
                </span>
                <code className="font-mono text-sm text-white/70 select-all">
                  npx arcanea init
                </code>
              </div>
