  ...
  return (
    ...
    >
      <Link href={href} className="group block h-full">
        <div
          className={`relative h-full p-8 rounded-3xl border border-white/10 bg-cosmic-surface/30 hover:${hoverColor} hover:bg-cosmic-surface/50 transition-all duration-300 hover:-translate-y-1`}
        >
          {/* Glow effect */}
          <div
            className="absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-300"
            style={{
              background: `radial-gradient(ellipse at 50% 0%, ${accentColor}15, transparent 60%)`,
            }}
          />

          <div className="relative z-10">
            <div
              className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${accentColor}/20 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300`}
            >
              {icon}
            </div>
            <h3
              className={`font-display text-xl font-semibold mb-3 group-hover:text-${accentColor} transition-colors`}
            >
              {title}
            </h3>
