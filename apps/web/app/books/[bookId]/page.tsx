...
> = {
  "in-progress": {
    bg: "bg-[var(--arc-brand-atlantean-teal)]/15 border-[var(--arc-brand-atlantean-teal)]/30",
...
async function getCompanionFlags(
  bookDir: string,
): Promise<{ hasAuthorsNote: boolean; hasGlossary: boolean }> {
  // Book root is one level up from chapters/ — fall back to bookDir if no /chapters suffix
  const root =
    bookDir.endsWith("chapters") ||
    bookDir.endsWith("chapters/") ||
    bookDir.endsWith("chapters\\")
      ? join(bookDir, "..")
      : bookDir;
  ...
  return (
    <div className="relative min-h-screen bg-[var(--arc-cosmic-void)]">
      ...
      <main className="relative z-10 mx-auto max-w-3xl px-6 pb-24 pt-16">
        ...
        {/* Decorative divider */}
        <div
          className="mb-10 flex items-center gap-3 text-[var(--arc-brand-atlantean-teal)]/20"
          aria-hidden="true"
        >
        ...
        {hasCompanion && (
          <div className="mt-16 border-t border-white/[0.06] pt-10">
            ...
            >
              ...
              <svg
                className="h-4 w-4 text-[var(--arc-brand-atlantean-teal)]/50 transition-colors group-hover:text-[var(--arc-brand-atlantean-teal)]"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={2}
