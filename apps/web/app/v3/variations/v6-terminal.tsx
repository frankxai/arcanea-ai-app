...
const C = {
  ...
  yellow: "var(--arc-brand-arcanean-gold)",
  violet: "var(--arc-void)",
  white: "var(--arc-text-primary)",
  dim: "var(--arc-earth)",
  border: "var(--arc-cosmic-void)",
  hoverBg: "rgba(74,222,128,0.06)",
  dotGreen: "var(--arc-earth)",
};
...
const CMDS = [
  ...
  {
    cmd: "library search",
    desc: "Browse 57 texts of creation wisdom",
    href: "/library",
  },
  {
    cmd: "forge create",
    desc: "Generate art, stories, music",
    href: "/studio",
  },
  {
    cmd: "gateway models",
    desc: "List 26 curated AI models",
    href: "/gateway",
  },
  ...
  if (!t)
    return (
      <div
        style={{
          height: "1.4em",
          animation: "reveal .15s ease forwards",
    ...
    <div style={{ color: C.border, whiteSpace: "pre", lineHeight: "1.5" }}>
      ...
      <div>
        ...
        <span style={{ color: C.white, fontWeight: 700 }}>
          {pad("ARCANEA INTELLIGENCE GATEWAY", w - 3)}
        </span>
        <span>{"\u2551"}</span>
      </div>
      <div>{mid}</div>
...
function Roster() {
  ...
  const row = (r: typeof hdr, h: boolean) => (
    <div
      ...
      onMouseEnter={(e) => {
        if (!h) (e.currentTarget as HTMLElement).style.background = C.hoverBg;
      }}
      onMouseLeave={(e) => {
        (e.currentTarget as HTMLElement).style.background = "transparent";
      }}
    >
      <span style={{ color: h ? C.dim : C.yellow, width: "3.5rem" }}>
        {pad(r.pid, 6)}
      </span>
      <span style={{ color: h ? C.dim : C.violet, width: "10rem" }}>
        {pad(r.name, 14)}
      </span>
      <span style={{ color: h ? C.dim : C.white, width: "7rem" }}>
        {pad(r.el, 10)}
      </span>
      <span style={{ color: h ? C.dim : C.cyan, width: "7rem" }}>
        {pad(r.freq, 9)}
      </span>
  ...
  return (
    <div style={{ lineHeight: "1.6" }}>
      {row(hdr, true)}
      <div
        style={{ height: "1px", background: C.border, margin: ".25rem 0" }}
      />
  ...
  return (
    ...
    >
      ...
      <span style={{ color: C.cyan }}>
        {"https://arcanea.ai/api/v1/chat/completions"}
      </span>
      ...
      >
        ...
        >
          ...
          <section style={{ marginBottom: "5rem", paddingTop: "2rem" }}>
            {BOOT.map((l, i) => (
              <Line key={i} t={l.t} c={l.c} b={l.b} i={i} />
            ))}
            <span
              style={{
                color: C.green,
                fontWeight: 700,
                animation:
                  "reveal .15s ease forwards, blink 1s step-end infinite",
