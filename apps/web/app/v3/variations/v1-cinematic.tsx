...
function GlassBtn({
  ...
  const border =
    color === S.teal ? "rgba(127,255,212,0.2)" : "rgba(197,165,90,0.25)";
  const bg =
    color === S.teal ? "rgba(127,255,212,0.04)" : "rgba(197,165,90,0.04)";
  const hBg =
    color === S.teal ? "rgba(127,255,212,0.1)" : "rgba(197,165,90,0.1)";
  const hBorder =
    color === S.teal ? "rgba(127,255,212,0.4)" : "rgba(197,165,90,0.45)";
  return (
    <a
      href={href}
...
function CinemaPanel({
  ...
  return (
    <section
      ...
      style={{ minHeight: "80vh", padding: "80px 0" }}
    >
      <div
        className={`mx-auto w-full max-w-6xl px-6 flex ${fd} items-center gap-16 flex-wrap lg:flex-nowrap`}
      >
        <div className={`flex-1 min-w-[280px] ${ta}`}>
          ...
          <p
            className="mt-5"
            style={{
              fontFamily: S.crimson,
              fontSize: "18px",
              color: "rgba(232,230,225,0.6)",
              lineHeight: 1.7,
              maxWidth: align === "center" ? "540px" : "440px",
              margin: align === "center" ? "20px auto 0" : undefined,
            }}
          >
            {subtitle}
          </p>
...
export function V1Cinematic() {
  ...
  return (
    <div
      ref={revealRef}
      style={{ background: S.bg, color: S.text, minHeight: "100vh" }}
    >
      {/* Film grain */}
      <div
        aria-hidden
      ...
      >
        ...
        {(["top", "bottom"] as const).map((pos) => (
          <div
            key={pos}
            className={`absolute left-0 right-0 z-20 ${pos === "top" ? "top-0" : "bottom-0"}`}
            style={{
        ...
        <div
          ...
          style={{
            ...
            width: "700px",
            height: "400px",
            background: `radial-gradient(ellipse, rgba(127,255,212,0.12) 0%, rgba(127,255,212,0.03) 40%, transparent 70%)`,
        ...
        >
          ...
          <GlassBtn href="/studio" color={S.teal}>
            Enter <ArrowRight size={16} weight="bold" />
          </GlassBtn>
        </div>
      </section>
      ...
      <section className="reveal-item py-24">
        <div className="mx-auto max-w-6xl px-6">
          ...
          <div className="relative">
            <div
              ref={scrollRef}
              className="gs flex gap-6 overflow-x-auto pb-4"
              style={{ scrollSnapType: "x mandatory", padding: "0 20px" }}
            >
              {GUARDIANS.map((g) => {
                const Icon = g.icon;
                return (
                  <div
                    key={g.name}
                    className="flex-shrink-0 snap-center flex flex-col items-center justify-end"
                    style={{
                      width: "240px",
                      height: "320px",
                      background: S.glass,
      ...
      <CinemaPanel
        title="Forge"
        subtitle="Create with AI that understands narrative, mythology, and world-building. From characters to cosmologies, shape living universes."
        align="left"
      >
        <MockupBox>
          <div style={{ color: "rgba(127,255,212,0.6)" }}>
            {"// forge a new world"}
          </div>
          <div>
            <span style={{ color: S.gold }}>const</span>{" "}
            <span style={{ color: S.text }}>world</span>
            {" = "}
            <span style={{ color: S.gold }}>await</span>{" "}
            <span style={{ color: S.teal }}>forge</span>
            {"({"}
          </div>
          <div className="pl-4">
            {"name: "}
            <span style={{ color: S.teal }}>{'"Eldara"'}</span>,
          </div>
          <div className="pl-4">
            {"element: "}
            <span style={{ color: S.teal }}>{'"fire"'}</span>,
          </div>
          <div className="pl-4">
            {"guardian: "}
            <span style={{ color: S.teal }}>{'"Draconia"'}</span>
          </div>
          <div>{"});"}</div>
        </MockupBox>
      </CinemaPanel>
      ...
      <CinemaPanel
        title="Library"
        subtitle="57 wisdom texts across 20 collections. Laws, parables, prophecies, and meditations. Ancient frameworks for modern creators."
        align="center"
      >
        <div className="relative w-full" style={{ height: "240px" }}>
          {[
            { t: "10%", l: "15%", d: "0s", s: 24 },
            { t: "25%", r: "20%", d: "1.5s", s: 20 },
            { b: "30%", l: "25%", d: "0.8s", s: 22 },
            { t: "60%", r: "10%", d: "2.2s", s: 18 },
            { b: "15%", l: "45%", d: "1.2s", s: 26 },
          ].map((p, i) => (
            <div
              key={i}
              className="absolute"
              style={{
                ...
                left: p.l,
                right: p.r,
                animation: `floatBook 6s ease-in-out ${p.d} infinite`,
              }}
            >
              <BookOpen
                size={p.s}
      ...
      <CinemaPanel
        title="Gateway"
        subtitle="26 AI models behind a single API. Claude, Gemini, GPT, and more. One endpoint, every intelligence."
        align="right"
      >
        <MockupBox>
          ...
          <div className="pl-4">
            {'"model": '}
            <span style={{ color: S.teal }}>{'"claude-opus"'}</span>,
          </div>
          <div className="pl-4">
            {'"prompt": '}
            <span style={{ color: S.teal }}>
              {'"Describe the Gate of Fire"'}
            </span>
          </div>
          <div>{"}"}</div>
          <div className="mt-3" style={{ color: "rgba(127,255,212,0.4)" }}>
            {"// 26 models. One endpoint."}
          </div>
        </MockupBox>
      </CinemaPanel>
      ...
      <section className="reveal-item flex flex-col items-center justify-center py-32 px-6">
        ...
        <GlassBtn href="/discover" color={S.gold}>
          Start Creating <ArrowRight size={16} weight="bold" />
        </GlassBtn>
      </section>

      {/* FOOTER */}
      <footer
        className="flex flex-col sm:flex-row items-center justify-between gap-4 px-6 py-8 mx-auto max-w-6xl"
        style={{
