...
export function VaultEntryCard({ entry }: { entry: VaultEntry }) {
  const config = VAULT_CONFIG[entry.vault as VaultCategory];

  return (
    <div className="group relative bg-white/[0.03] border border-white/[0.06] backdrop-blur-sm rounded-xl p-4 transition-all duration-300 hover:bg-white/[0.05] hover:border-white/[0.1]">
      <div
        className="absolute left-0 top-3 bottom-3 w-0.5 rounded-full"
        style={{ backgroundColor: config?.color ?? "var(--arc-earth)" }}
      />
      <div className="pl-3">
        <div className="flex items-start justify-between gap-3 mb-2">
          <p className="text-sm text-[var(--arc-text-primary)] leading-relaxed flex-1">
            {entry.content}
          </p>
        ...
        <div className="flex items-center gap-3 text-[10px] text-[var(--arc-text-muted)]">
          ...
          {entry.tags?.length > 0 &&
            entry.tags.map((tag) => (
              <span
                key={tag}
                className="font-mono px-1.5 py-0.5 rounded bg-white/[0.04]"
              >
                {tag}
              </span>
            ))}
