...
export const BLOG_POSTS: BlogPost[] = [
  {
    slug: 'ai-model-arena-free-models-guide',
    title: 'The AI Model Arena: How We Route Free Models Across 10 Agents',
    excerpt:
...
We think this is wrong. If you are building with AI — if your creative work, your code, your world-building depends on these systems — you deserve to know exactly what is running, why it was chosen, and what it costs. Arcanea publishes its full model stack. Every agent, every model, every routing decision. This post explains the architecture.
...
**Qwen 3.6 Plus** — Alibaba's flagship. 1M token context window, best-in-class agentic reasoning, strong multilingual performance. This is the model you reach for when you need an agent that can hold an entire codebase in context and reason about it coherently. It handles orchestration tasks that would choke smaller models.

**MiniMax M2.5** — The coding specialist. Highest SWE-Bench score among free models at time of writing. Exceptional at code generation, refactoring, and debugging. Where Qwen thinks broadly, MiniMax cuts precisely.

**Kimi K2.5** — Moonshot AI's reasoning model. Strong at mathematical reasoning, structured analysis, and tasks that require chains of logical deduction. When a task needs careful step-by-step thinking rather than broad pattern matching, Kimi delivers.

**GLM 4.7** — Zhipu AI's general-purpose model. Balanced across tasks with particularly strong creative writing capabilities. Good at generating narrative content that reads naturally rather than mechanically.

**Big Pickle** — The wildcard. Competitive performance on general benchmarks with surprisingly strong creative and conversational abilities. We use it for tasks where personality and tone matter more than raw reasoning.

**GPT-5 Nano** — OpenAI's lightweight model in the free tier. Fast inference, good at simple transforms, and reliable for structured output. The workhorse for tasks that need speed over depth.
...
First, "best" depends on the task. A model that excels at code generation may produce mediocre creative writing. A model with a 1M context window is overkill for a 200-token classification task.

Second, latency matters. If an agent needs a quick decision to unblock a pipeline, waiting 5 seconds for a large model when a small model could answer in 200ms is a design failure.

Third, diversity is resilience. If your entire system depends on one model and that model's API goes down, your system goes down. Routing across multiple providers means no single point of failure.
...
Here is the full agent-to-model mapping with the reasoning behind each assignment:

| Agent | Role | Model | Why |
|-------|------|-------|-----|
| **Sisyphus** | Orchestrator | Qwen 3.6 Plus | 1M context holds the full project state. Best agentic reasoning for coordination. |
| **Hephaestus** | Coder | MiniMax M2.5 | Highest SWE-Bench. Writes code that passes tests on the first attempt. |
| **Athena** | Researcher | Kimi K2.5 | Strong chain-of-thought for synthesizing information across sources. |
| **Hermes** | Messenger | GPT-5 Nano | Fast inference for routing decisions and structured metadata. |
| **Apollo** | Creative Writer | GLM 4.7 | Natural prose that does not read like it was generated by a machine. |
| **Hestia** | Reviewer | Qwen 3.6 Plus | Long context lets it review entire PRs. Catches architectural issues. |
| **Prometheus** | Architect | Kimi K2.5 | Structured reasoning for system design and dependency analysis. |
| **Dionysus** | Social/Content | Big Pickle | Personality-forward output for community content and social posts. |
| **Ares** | Security | MiniMax M2.5 | Precise pattern matching catches vulnerabilities that creative models miss. |
| **Demeter** | Data/Analytics | GPT-5 Nano | Fast structured output for data transforms and metric calculations. |

This table is not static. We re-evaluate weekly based on benchmark updates, new model releases, and observed production performance.
...
**SWE-Bench Verified** — Can the model fix real software bugs? This is the single best predictor of coding agent quality. MiniMax M2.5 leads here.

**Context Window (Effective)** — Not the advertised maximum, but how well the model actually uses long context. Many models claim 128K but degrade past 32K. Qwen 3.6 Plus genuinely uses its full 1M window.

**Inference Speed** — Time to first token and tokens per second. Critical for agents in the hot path. GPT-5 Nano wins here by a wide margin.

**Instruction Following** — Does the model do exactly what you asked, or does it add unsolicited commentary, refuse valid requests, or hallucinate features? Measured by our internal compliance benchmark.

**Creative Quality** — Human-evaluated output quality for narrative, dialogue, and world-building content. GLM 4.7 and Big Pickle lead here.

**Cost** — All models listed are free tier. But rate limits differ. We track effective throughput per hour to avoid hitting limits during high-intensity sessions.

We publish updated rankings on the /models page every week.

## How to Use This Yourself

Everything described here is reproducible. The model routing runs through OpenCode Zen, and the configuration is open.

Visit [/models](/models) for the live arena with current benchmarks, rankings, and model cards.
...
routing:
  strategy: task-matched
  fallback: orchestrator
  retry_on_failure: true
  max_retries: 2
\`\`\`

The \`task-matched\` strategy inspects the task type and routes to the model whose profile fits. Fallback ensures that if a model is unavailable, the orchestrator model handles the request.

## Weekly Updates

The AI model landscape shifts fast. New models drop weekly. Benchmarks update. Free tiers change. We track all of it.

The [/models](/models) page updates every week with:
- New model additions and removals
- Updated benchmark scores
- Routing table changes
- Cost and rate limit updates
