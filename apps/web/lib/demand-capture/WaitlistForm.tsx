...
export const DEFAULT_COPY: WaitlistCopy = {
  ...
  intro:
    "Three optional questions. They decide what gets built first, and what it costs.",
  roleLabel: "You are",
  ...
  consent:
    "Email me about {product}. Your address and your answers stay with this list. Unsubscribe in any email.",
};
...
export function WaitlistForm({
  ...
  // Humans never see this field; a non-empty value is refused server-side.
  ...
  async function post(payload: Record<string, unknown>) {
    ...
    try {
      ...
      setError(
        e instanceof Error
          ? e.message
          : "Something went wrong. Your email was not saved.",
      );
      return false;
    } finally {
      setBusy(false);
  ...
  const seats =
    state?.foundingSeatsLeft ?? initialState?.foundingSeatsLeft ?? null;
  ...
  if (step === "join") {
    return (
      ...
      >
        ...
        <div aria-hidden="true" className="wl-trap">
          <label htmlFor={uid + "-trap"}>Leave this empty</label>
          <input
            id={uid + "-trap"}
            type="text"
            name={HONEYPOT_FIELD}
        ...
        <p className="wl-note">
          {shownCount !== null
            ? fill(copy.waiting, { count: shownCount.toLocaleString() }) + " "
            : ""}
      ...
      <div className="space-y-6">
        ...
        >
          <p className="wl-panel-title">
            {state?.position
              ? fill(copy.entered, { n: state.position })
              : copy.enteredNoPosition}
          </p>
          {foundingBenefit && seats !== null && seats > 0 ? (
            <p className="wl-panel-text">
              {fill(copy.seats, { n: seats, benefit: foundingBenefit })}
            </p>
          ) : null}
        </div>
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              value={answers.pain ?? ""}
              onChange={(event) =>
                setAnswers({ ...answers, pain: event.target.value })
              }
  ...
  return (
    ...
    >
      <p className="wl-panel-title">
        {state?.position
          ? fill(copy.done, { n: state.position })
          : copy.doneNoPosition}
      </p>
      <p className="wl-panel-text">
        {fill(copy.doneText, { product: productName })}
      </p>
    </div>
  );
}
