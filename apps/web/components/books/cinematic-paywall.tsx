...
export function CinematicPaywall({
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#06080d] px-6 py-20 text-[#f4efe4]">
      <div
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_52%_10%,rgba(178,126,62,0.18),transparent_34%),radial-gradient(circle_at_12%_70%,rgba(35,116,125,0.16),transparent_30%)]"
        aria-hidden="true"
      />
      <div className="relative mx-auto max-w-2xl">
        <Link
          href={`/books/${CINEMATIC_BOOK_ID}`}
          className="text-sm text-[#b4c9c5] transition hover:text-white"
        >
          ← {CINEMATIC_BOOK_TITLE}
        </Link>
        ...
        <section className="mt-16 border-y border-white/10 py-12">
          ...
          <p className="mt-8 max-w-xl font-serif text-lg leading-8 text-white/65">
            {released
              ? `The free opening ends after Chapter ${FREE_CHAPTER_COUNT}. The complete founding edition includes all 32 chapters, the responsive reader, EPUB and PDF ownership files, the cinematic artbook, and the documented production ledger.`
          ...
          <div className="mt-10 flex flex-col items-start gap-4 sm:flex-row sm:items-center">
            ...
            <Link
              href={`/books/${CINEMATIC_BOOK_ID}/01-the-house-that-leaned`}
          ...
          {verifying ? (
            <p className="mt-5 text-sm leading-6 text-amber-100/70">
              We could not verify Polar access just now. Nothing has been
              charged here; refresh once the service is available.
            </p>
          ) : null}
        </section>
      </div>
