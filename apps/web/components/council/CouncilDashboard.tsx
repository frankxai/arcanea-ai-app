...
export function CouncilDashboard() {
  const [convenings, setConvenings] = useState<Convening[]>([]);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Not authenticated or no council yet — show defaults
      } finally {
        setLoading(false);
      }
    }
    ...
    <>
      ...
      <section className="max-w-3xl mx-auto">
        <CouncilStats
          ...
          totalConvenings={stats?.total_convenings}
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] px-6 py-10 text-center">
            <p className="font-body text-sm text-white/40">
              No convenings yet. Begin your first session to see your journal
              here.
            </p>
          </div>
        )}
