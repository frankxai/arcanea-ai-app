# Resonant — v0 build brief

Attach resonant.html alongside this brief. The HTML is the visual and interaction reference; build an idiomatic Next.js App Router implementation with strict TypeScript and the destination project's existing design tokens. The starter itself is standalone HTML, not pre-generated Next.js source.

## Audience and job

Producer portfolio: An editorial artist page with a playable record sleeve, track selector and release notes.

## Art direction and behavior

Build a music producer portfolio with an asymmetric record-sleeve hero, generous editorial typography, three-track player, release notes and artist statement. Keep track ownership and download permissions explicit. The built-in audio is synthesized demo sound, not a commercial catalog. Replace it with owner-provided licensed tracks before launch.

## Required sections

- Record sleeve and track selector
- Selected releases with liner notes
- Artist statement and release credits

## Inputs to replace before release

- Artist name and one-sentence sound identity
- Owned audio, artwork and release credits
- Verified streaming or contact destinations
