...
The live `/api/ai/chat` health endpoint reports all server provider keys absent, including OpenAI. The reader route intentionally returns 503 before charging when `OPENAI_API_KEY` is missing. Configure that secret in Vercel with a provider spend cap, or select a budgeted AI Gateway deployment and change the route explicitly. No server image generation can be claimed until a real authenticated call, saved asset and credit settlement succeed.
