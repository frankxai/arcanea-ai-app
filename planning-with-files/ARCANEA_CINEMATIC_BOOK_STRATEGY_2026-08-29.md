...
Next actions: verify the integrated revision on a protected Vercel preview; close mobile cover placement, reader size labels, and assistant overlap; integrate the shared per-product waitlist; then direct one opening proof sequence before expanding the art set. Use Arion's damaged street, Mera's rain record, and Emilia's prism bench for environment, performance, and material-detail frames. The cover study needs a thumbnail-legible composition and a separately typeset title/byline before commercial approval. Independent editorial, visual, commerce, and release reviews remain required. The four-chapter preview is the current implementation; reconcile it with Frank's originally requested one-chapter paywall before release.
