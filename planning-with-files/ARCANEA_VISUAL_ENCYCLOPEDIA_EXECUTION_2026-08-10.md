...
- Remote hosting is paused at the shared-platform release gate. Arcanea PR #243 consumes governed receipts; Agentic Ops PR #14 defines Cloudflare R2 plus a dedicated shared Supabase registry as canonical. Neither draft PR has been merged or provisioned by this lane.
...
- The gallery no longer lists a bucket or needs a write token. Before media activation it uses reviewed local metadata. Once configured, it accepts only a `starlight.media-publication-receipt.v1` file from `https://media.starlightintelligence.org` with matching checksums, content-addressed URL/key, registry evidence, and explicit withdrawal tombstones. Invalid or unavailable governed receipts fail build/revalidation instead of resurrecting withdrawn local entries.
- Agentic Ops now carries the reversible lifecycle foundation: a separate `MEDIA_LIFECYCLE_TOKEN`, rendition-scoped independent withdrawal review, non-destructive registry revoke/archive, retained content-addressed bytes, append-only event evidence, and deterministic receipt tombstones. The Arcanea contract removes a visual only after validating its withdrawal IDs, source checksum, and time; duplicate, unknown, overlapping, or drifted tombstones fail closed.
- Vercel preview proof is READY at the protected branch preview, with 130 dossier routes generated and active checks green. Public `arcanea.ai/gallery` still serves the earlier collection because both draft PRs, media provisioning, receipt attachment, and production promotion remain human-gated.
