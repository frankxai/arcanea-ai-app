...
- A deterministic Media Fabric intake packet now covers all 130 masters: 320,204,312 bytes, unique SHA-256 checks, exact PNG dimensions, stable registry UUIDs, proposed R2 delivery keys, provenance, usage links, and explicit rights/review/publication holds. The build performed zero network writes.
- The gallery no longer lists a bucket or needs a write token. It fails closed to catalog metadata until a `starlight.media-publication-receipt.v1` file arrives from `https://media.starlightintelligence.org` with matching IDs, URLs, and checksums.
