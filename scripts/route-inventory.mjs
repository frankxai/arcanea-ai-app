...
const SCAN_DIRS = ['app', 'components', 'lib', 'content', 'data', 'messages', 'hooks', 'i18n'];
...
function topLevelRoutes() {
  ...
  for (const name of readdirSync(APP)) {
    ...
    if (!statSync(p).isDirectory() || name === 'api' || name.startsWith('_') || name.startsWith('[')) continue;
    if (name.startsWith('(')) {
      for (const child of readdirSync(p)) {
        if (statSync(join(p, child)).isDirectory()) routes.push({ segment: child, dir: join(p, child) });
      }
      continue;
    }
...
const refPattern = (segment, flags = 'g') => new RegExp(`(?:["'\`(=}]|\\]\\()/${escape(segment)}(?=[/"'\`?#)\\s]|$)`, flags);

function lastTouch(dir) {
  try {
    return execFileSync('git', ['log', '-1', '--format=%cs', '--', relative(ROOT, dir)], { cwd: ROOT, encoding: 'utf8' }).trim() || null;
  } catch {
    return null;
  }
}
...
const inventory = topLevelRoutes().map(({ segment, dir }) => {
  ...
  const nav = inbound.filter((r) => NAV_FILES.includes(r.file)).map((r) => r.file);
  return {
    route: `/${segment}`,
    ...
    inNav: nav.length > 0,
    redirected: routeRedirects.some((r) => r.source === `/${segment}` || r.source === `/${segment}/:path*`),
...
const rows = args.has('--unlinked') ? inventory.filter((r) => r.inboundFiles === 0 && !r.inSitemap) : inventory;

if (args.has('--json')) {
  console.log(JSON.stringify(rows.map(({ inbound, ...r }) => ({ ...r, inbound: inbound.map((i) => i.file) })), null, 2));
} else {
  console.log(['route', 'page', 'inboundFiles', 'refs', 'sitemap', 'nav', 'lastTouch'].join('\t'));
  for (const r of rows) {
    console.log([r.route, r.hasPage ? 'y' : '-', r.inboundFiles, r.inboundRefs, r.inSitemap ? 'y' : '-', r.inNav ? 'y' : '-', r.lastTouch ?? '?'].join('\t'));
  }
  console.log(`\n${rows.length} routes`);
}
