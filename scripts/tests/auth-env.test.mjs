...
test("production refuses the missing public binding that disabled Google sign-in", () => {
  assert.deepEqual(
    validateAuthBuildEnvironment({ VERCEL_ENV: "production" }).errors,
    [
...
test("offline and unconfigured preview builds remain available", () => {
  ...
  assert.equal(
    validateAuthBuildEnvironment({ VERCEL_ENV: "preview" }).required,
    false,
  );
  assert.equal(
    validateAuthBuildEnvironment({ VERCEL_ENV: "preview", CHECK_AUTH_ENV: "1" })
      .errors.length,
    2,
  );
});
test("existing anon and modern publishable keys are accepted in the app binding", () => {
  ...
  assert.deepEqual(
    validateAuthBuildEnvironment({
      ...configured,
      NEXT_PUBLIC_SUPABASE_ANON_KEY: "sb_publishable_examplevalidkey123456789",
    }).errors,
    [],
  );
});
test("placeholder, malformed, and privileged keys never pass a public build gate", () => {
  ...
  for (const key of [
    "preview-build-placeholder",
    "malformed",
    "sb_secret_neverpublic",
    privileged,
    ` ${anon}`,
  ]) {
...
test("example, insecure, credential-bearing, or non-origin URLs are rejected", () => {
  for (const value of [
    ...
    "http://demo-ref.supabase.co",
    "https://user:pass@demo-ref.supabase.co",
    `${url}/auth`,
    `${url}?x=1`,
    `${url}#fragment`,
    "broken",
...
test("the command fails before a production build and never prints secrets", () => {
  const result = spawnSync(process.execPath, ["scripts/check-auth-env.mjs"], {
    ...
    env: {
      VERCEL_ENV: "production",
      NEXT_PUBLIC_SUPABASE_ANON_KEY: "sb_secret_donotprint",
    },
  });
  assert.equal(result.status, 1);
  assert.match(result.stderr, /NEXT_PUBLIC_SUPABASE_URL/);
  assert.ok(
    !`${result.stdout}${result.stderr}`.includes("sb_secret_donotprint"),
  );
});
