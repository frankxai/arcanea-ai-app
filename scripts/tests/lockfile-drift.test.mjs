...
test('metadata and script edits never require a lockfile change', () => {
  const after = { ...base, version: '1.1.0', description: 'new', scripts: { build: 'tsc', test: 'node --test' }, keywords: ['mcp'] };
...
test('any dependency field change requires a lockfile change', () => {
  for (const field of ['dependencies', 'devDependencies', 'peerDependencies', 'optionalDependencies', 'peerDependenciesMeta', 'dependenciesMeta', 'bundledDependencies', 'overrides', 'resolutions', 'pnpm']) {
    const after = { ...base, [field]: { ...(base[field] ?? {}), added: '1.0.0' } };
...
test('the change PR #388 made (description + scripts) passes', () => {
  ...
  const after = { ...base, description: 'Local Arcanea worldbuilding MCP tools', scripts: { test: 'node --test tests/*.test.mjs', prepack: 'tsc', prepublishOnly: 'tsc' } };
  assert.equal(needsLockfileChange(before, after), false);
});
