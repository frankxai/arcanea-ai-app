...
export function needsLockfileChange(before, after) {
  ...
  return DEPENDENCY_FIELDS.some(
    (field) =>
      JSON.stringify(canonical(before[field])) !==
      JSON.stringify(canonical(after[field])),
  );
}
...
function main([baseRef, headRef]) {
  if (!baseRef || !headRef) {
    console.error(
      "Usage: node scripts/lockfile-drift.mjs <base-sha> <head-sha>",
    );
    return 1;
  }
  ...
  console.log(
    `dependency changes: ${drifting.length}${drifting.length ? ` (${drifting.join(", ")})` : ""}`,
  );
  console.log(`pnpm-lock.yaml changed: ${lockChanged ? 1 : 0}`);
  if (process.env.GITHUB_OUTPUT) {
    appendFileSync(
      process.env.GITHUB_OUTPUT,
      `pkg_changed=${manifests.length}\ndeps_changed=${drifting.length}\nlock_changed=${lockChanged ? 1 : 0}\n`,
    );
  }

  if (drifting.length > 0 && !lockChanged) {
    console.log(
      `::error::Dependencies changed in ${drifting.join(", ")} but pnpm-lock.yaml did not. Run pnpm install and commit pnpm-lock.yaml.`,
    );
    return 1;
  }
  return 0;
