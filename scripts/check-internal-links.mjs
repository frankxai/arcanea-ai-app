...
function vercelSources() {
  return ["vercel.json", "apps/web/vercel.json"]
    ...
    .flatMap((f) => {
      ...
      return [...(cfg.redirects ?? []), ...(cfg.rewrites ?? [])].map(
        (r) => r.source,
      );
    });
}
...
const isPublicFile = (path) =>
  existsSync(join(WEB, "public", decodeURIComponent(path)));
const resolves = (path) =>
  path.startsWith("/_next/") ||
  isPublicFile(path) ||
  served.some((re) => re.test(path));
...
const files = useBuilt
  ...
  : ["app", "components", "lib"].flatMap((d) =>
      walk(
        join(WEB, d),
        (n) => /\.(tsx?|jsx?|mdx?)$/.test(n) && !/\.(test|spec)\./.test(n),
      ),
    );
...
const topLevel = new Set(
  readdirSync(APP).map((n) => n.replace(/^\((.*)\)$/, "$1")),
);
for (const group of readdirSync(APP).filter((n) => n.startsWith("(")))
  readdirSync(join(APP, group)).forEach((n) => topLevel.add(n));
...
const print = (log, list) =>
  list.forEach(([path, where]) =>
    log(`  ${path}  <- ${[...new Set(where)].join(", ")}`),
  );

console.log(
  `Scanned ${files.length} ${useBuilt ? "built" : "source"} files against ${served.length} route/redirect patterns.`,
);
if (warnings.length) {
  console.warn(
    `Warning: ${warnings.length} unresolved deeper path(s) under live routes (dynamic data or pre-existing):`,
  );
  print(console.warn, warnings);
}
if (missing.length) {
  console.error(
    `FAIL: ${missing.length} link(s) target a route family with no page, public file, or redirect:`,
  );
  print(console.error, missing);
  process.exit(1);
}
console.log(
  "Internal links: no link targets a removed route without a redirect.",
);
